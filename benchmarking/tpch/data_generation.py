...
def gen_csv_files(basedir: str, num_parts: int, scale_factor: float) -> str:
    ...
    """
    basedir_with_scale_factor = os.path.join(basedir, ("%.1f" % scale_factor).replace(".", "_"))
    if not os.path.exists(basedir_with_scale_factor):
        ...
        subprocess.check_output(
            shlex.split(f"git clone https://github.com/electrum/tpch-dbgen {basedir_with_scale_factor}")
        )
        subprocess.check_output("make", cwd=basedir_with_scale_factor)

        logger.info(f"Generating {num_parts} for tpch")
        if num_parts == 1:
            subprocess.check_output(shlex.split(f"./dbgen -v -f -s {scale_factor}"), cwd=basedir_with_scale_factor)
        else:
            ...
            for part_idx in range(1, num_parts + 1):
                ...
                subprocess.check_output(
                    ...
                    cwd=basedir_with_scale_factor,
                )

        # The tool sometimes generates weird files with bad permissioning, we override that manually here
        subprocess.check_output(shlex.split("chmod -R u+rwx ."), cwd=basedir_with_scale_factor)

    return basedir_with_scale_factor
