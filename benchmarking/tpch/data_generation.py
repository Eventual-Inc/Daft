...
TPCH_DBGEN_CFLAGS = (
    "-g -std=gnu89 -Wno-error=incompatible-pointer-types -Wno-error=int-conversion "
    '-DDBNAME="dss" -DMAC -DORACLE -DTPCH -DRNG_TEST -D_FILE_OFFSET_BITS=64'
)


def build_tpch_dbgen(cachedir: str) -> None:
    subprocess.check_output(["make", f"CFLAGS={TPCH_DBGEN_CFLAGS}"], cwd=cachedir)


def has_generated_tpch_data(cachedir: str, num_parts: int) -> bool:
    expected_files = [f"{table}.tbl" for table in STATIC_TABLES]
    if num_parts == 1:
        expected_files.extend(f"{table}.tbl" for table in PARTITIONED_TABLES)
    else:
        expected_files.extend(
            f"{table}.tbl.{part_idx}" for table in PARTITIONED_TABLES for part_idx in range(num_parts)
        )

    return os.path.isfile(os.path.join(cachedir, "dbgen")) and all(
        os.path.isfile(os.path.join(cachedir, path)) for path in expected_files
    )
...
def gen_csv_files(basedir: str, num_parts: int, scale_factor: float) -> str:
    ...
    if not has_generated_tpch_data(cachedir, num_parts):
        if os.path.exists(cachedir):
            logger.info(
                "Cached directory for scale_factor=%s num_parts=%s is incomplete at %s, regenerating",
                scale_factor,
                num_parts,
                cachedir,
            )
