...
def gen_sqlite_db(csv_filepath: str, num_parts: int) -> str:
    ...
    if not os.path.exists(sqlite_db_file_path):
        ...
        backup_files = local_fs.expand_path(f"{csv_filepath}/*.bak")
...
def gen_csv_files(basedir: str, num_parts: int, scale_factor: float) -> str:
    ...
    if not os.path.exists(cachedir):
        ...
        subprocess.check_output(
            shlex.split(f"git clone https://github.com/electrum/tpch-dbgen {cachedir}")
        )
        ...
        logger.info(f"Cached directory for scale_factor={scale_factor} num_parts={num_parts} already exists at {cachedir}, skipping CSV file generation")
...
def gen_parquet(csv_files_location: str) -> str:
    ...
    for tab_name in SCHEMA.keys():
        ...
        if not os.path.exists(table_parquet_path):
            ...
            logger.info(f"Cached Parquet files for table {tab_name} already exists at {table_parquet_path}, skipping Parquet file generation")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--generate_sqlite_db", action="store_true", help="Whether to generate a SQLite DB or not, defaults to False")
    parser.add_argument("--generate_parquet", action="store_true", help="Whether to generate Parquet files or not, defaults to False")
    args = parser.parse_args()
    num_parts = math.ceil(args.scale_factor) if args.num_parts is None else args.num_parts

    logger.info(f"Generating data at {args.tpch_gen_folder} with: scale_factor={args.scale_factor} num_parts={num_parts} generate_sqlite_db={args.generate_sqlite_db} generate_parquet={args.generate_parquet}")
