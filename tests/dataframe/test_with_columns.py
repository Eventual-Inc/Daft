...
def test_with_columns_aliased(make_df, valid_data: list[dict[str, float]]) -> None:
    df = make_df(valid_data)
    expanded_df = df.with_columns(
        [df["sepal_width"].alias("foo"), (df["sepal_width"] + df["petal_length"]).alias("bar")]
    )
...
def test_with_columns_tuple(make_df, valid_data: list[dict[str, float]]) -> None:
    df = make_df(valid_data)
...
def test_with_columns_mixed(make_df, valid_data: list[dict[str, float]]) -> None:
    df = make_df(valid_data)
    expanded_df = df.with_columns([df["sepal_width"].alias("foo"), ("bar", df["sepal_width"] + df["petal_length"])])
    data = expanded_df.to_pydict()
    assert expanded_df.column_names == df.column_names + ["foo", "bar"]
    assert data["foo"] == data["sepal_width"]
    assert data["bar"] == [sw + pl for sw, pl in zip(data["sepal_width"], data["petal_length"])]
