...
TEST_DATA_SCHEMA = pa.schema(
    [
        ...
        # BUG: Loading fixed-size-list data from Parquet fails
...
def test_to_arrow(make_df, n_partitions: int) -> None:
    df = make_df(TEST_DATA).repartition(n_partitions)
    table = df.to_arrow()
...
@pytest.mark.parametrize("n_partitions", [1, 2])
def test_to_pandas(make_df, n_partitions: int) -> None:
    df = make_df(TEST_DATA).repartition(n_partitions)
