...
def df():
    uids = [id for id in range(1024)]
    random.shuffle(uids)
    return daft.from_pylist([{"id": id, "name": f"user_{id}", "email": f"user_{id}@getdaft.io"} for id in uids])


def test_negative_limit(df):
...
def test_negative_offset(df):
    with pytest.raises(ValueError) as excinfo:
        df.select("name").offset(-1).limit(1).collect()
...
def test_offset_without_limit(df):
    with pytest.raises(Exception) as excinfo:
        df.select("name").offset(17).collect()
...
def test_limit_before_offset(df):
...
def test_limit_after_offset(df):
    ...
    for i in range(1, 517):
        df6 = df6.offset(1).select("name").offset(0)
...
def test_limit_before_offset_with_sort(df):
...
def test_limit_after_offset_with_sort(df):
