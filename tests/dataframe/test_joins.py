...
def skip_invalid_join_strategies(join_strategy, join_type):
    if context.get_context().daft_execution_config.enable_native_executor:
        if join_strategy not in [None, "hash"]:
            pytest.skip("Native executor only supports hash join")
        elif join_type == "outer":
            pytest.skip("Native executor does not support outer joins")
    else:
