...
class TestImageAverageHash:
    ...
    def test_average_hash_solid_colors(self):
        ...
        assert black_hash.count('0') > black_hash.count('1'), \
        ...
        assert white_hash.count('1') > white_hash.count('0'), \
    ...
    def test_average_hash_similarity(self):
        ...
        df = daft.from_pydict({
            "image_data": [base_img, similar_img, different_img],
            "type": ["base", "similar", "different"]
        })
        ...
        assert dist_base_similar < dist_base_different, \
        ...
        assert dist_base_different > 32, \
            f"Inverted images should have high Hamming distance, got {dist_base_different}"
    ...
    def test_average_hash_different_sizes(self):
        ...
        df = daft.from_pydict({
            "image_data": [small_img, medium_img, large_img],
            "size": ["4x4", "16x16", "64x64"]
        })
    ...
    def test_average_hash_grayscale_images(self):
        ...
        df = daft.from_pydict({
            "image_data": [dark_gray, light_gray],
            "type": ["dark", "light"]
        })
    ...
    def test_average_hash_batch_processing(self):
        ...
        df = daft.from_pydict({
            "image_data": images,
            "brightness": list(range(batch_size))
        })
...
def test_basic_functionality():
    ...
    df = daft.from_pydict({
        "image_data": [black_img, white_img],
        "name": ["black", "white"]
    })
