...
def test_sql_agg_default_naming():
    ...
    assert daft.sql("SELECT count(*) FROM df", **bindings).column_names == [
        "count"
    ]  # SQL count(*) is aliased to count by default in daft-sql
...
def test_sql_agg_supported_expressions_naming():
    ...
    assert daft.sql("SELECT bool_and(a IS NOT NULL) FROM df", **bindings).column_names == [
        "bool_and(NOT ((a) IS NULL))"
    ]
