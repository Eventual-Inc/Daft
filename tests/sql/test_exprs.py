...
def test_interval_comparison(date_values, ts_values, expected_intervals):
    ...
    df = daft.from_pydict({"date": date_values, "ts": ts_values}).select(
        col("date").cast(daft.DataType.date()), daft.functions.to_datetime(col("ts"), "%Y-%m-%d %H:%M:%S")
    )
    bindings = {"test": df}
...
def test_coalesce():
    ...
    expected = df.select(daft.functions.coalesce(col("a"), col("b"), col("c")).alias("result")).to_pydict()
