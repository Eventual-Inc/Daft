...
def test_filtered_sum(daft_df, pd_df):
    ...
    daft_df = daft_df.where(col("Borough") == "BROOKLYN").agg(col("Unique Key").sum().alias("unique_key_sum"))
...
def test_sum_groupby(daft_df, pd_df):
    """Sums across groups"""
    daft_df = daft_df.groupby(col("Borough")).agg(col("Unique Key").sum())
...
def test_sum_groupby_sorted(daft_df, sort_desc, pd_df):
    """Sums across groups"""
    daft_df = daft_df.groupby(col("Borough")).agg(col("Unique Key").sum()).sort(col("Borough"), desc=sort_desc)
