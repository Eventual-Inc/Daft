...
from daft.catalog.__unity._auth import OAuth2Credentials, OAuth2TokenProvider
...
def test_get_token_caching_logic(provider: OAuth2TokenProvider) -> None:
    with patch("daft.catalog.__unity._auth._generate_workspace_token") as mock_generate:
        with patch("daft.catalog.__unity._auth.jwt_expiration", return_value=9999999999):
            with patch("daft.catalog.__unity._auth.is_expired", return_value=False):
...
def test_get_token_refreshes_when_expired(provider: OAuth2TokenProvider) -> None:
    with patch("daft.catalog.__unity._auth._generate_workspace_token") as mock_generate:
        with patch("daft.catalog.__unity._auth.jwt_expiration", return_value=1000):
            mock_generate.return_value = "token_1"
            provider.get_token()

        with patch("daft.catalog.__unity._auth.is_expired", return_value=True):
            with patch("daft.catalog.__unity._auth.jwt_expiration", return_value=9999999999):
...
def test_get_token_failure_after_retries(provider: OAuth2TokenProvider) -> None:
    ...
    with patch("daft.catalog.__unity._auth._generate_workspace_token", side_effect=RuntimeError(error_msg)):
        with pytest.raises(RuntimeError, match="failed after 3 attempts"):
            provider.get_token()


def test_get_token_raises_if_token_is_none(provider: OAuth2TokenProvider) -> None:
    with patch("daft.catalog.__unity._auth.OAuth2TokenProvider._token_expired", return_value=True):
        with patch("daft.catalog.__unity._auth.OAuth2TokenProvider._refresh"):
