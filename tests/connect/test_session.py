...
class SparkSessionTestCase(unittest.TestCase):
    ...
    def test_creates_session_with_channel_builder(self):
        test_session = RemoteSparkSession.builder.channelBuilder(CustomChannelBuilder("sc://other")).getOrCreate()
...
class JobCancellationTests(ReusedConnectTestCase):
    ...
    def check_job_cancellation(self, setter, canceller, thread_ids, thread_ids_to_cancel, thread_ids_to_run):
        ...
        def run_job(job_id, index):
            ...
            try:
                ...
                self.spark.createDataFrame([[20]]).repartition(1).mapInPandas(func, schema="_1 LONG").collect()
        ...
        for i in thread_ids_to_cancel:
            self.assertTrue(is_job_cancelled[i], f"Thread {i}: Job in group A was not cancelled.")

        for i in thread_ids_to_run:
            self.assertFalse(is_job_cancelled[i], f"Thread {i}: Job in group B did not succeeded.")

    def test_inheritable_tags(self):
        self.check_inheritable_tags(create_thread=lambda target, session: InheritableThread(target, session=session))
        self.check_inheritable_tags(
            create_thread=lambda target, session: threading.Thread(target=inheritable_thread_target(session)(target))
        )
        ...
        self.check_inheritable_tags(create_thread=lambda target, session: threading.Thread(target=func, args=(target,)))
