...
class SparkConnectSQLTestCase(ReusedConnectTestCase, SQLTestUtils, PandasOnSparkTestUtils):
    ...
    def spark_connect_clean_up_test_data(cls):
        cls.spark.sql(f"DROP TABLE IF EXISTS {cls.tbl_name}")
        cls.spark.sql(f"DROP TABLE IF EXISTS {cls.tbl_name2}")
        cls.spark.sql(f"DROP TABLE IF EXISTS {cls.tbl_name3}")
        cls.spark.sql(f"DROP TABLE IF EXISTS {cls.tbl_name4}")
        cls.spark.sql(f"DROP TABLE IF EXISTS {cls.tbl_name_empty}")
...
class SparkConnectBasicTests(SparkConnectSQLTestCase):
    ...
    def test_with_columns_renamed(self):
        ...
        self.assertEqual(
            self.connect.read.table(self.tbl_name).withColumnsRenamed({"id": "id_new", "name": "name_new"}).schema,
            self.spark.read.table(self.tbl_name).withColumnsRenamed({"id": "id_new", "name": "name_new"}).schema,
        )
    ...
    def test_select_expr(self):
        ...
        self.assert_eq(
            self.connect.read.table(self.tbl_name).selectExpr(["id * 2", "cast(name as long) as name"]).toPandas(),
            self.spark.read.table(self.tbl_name).selectExpr(["id * 2", "cast(name as long) as name"]).toPandas(),
        )
...
class SparkConnectGCTests(SparkConnectSQLTestCase):
    ...
    def test_garbage_collection_checkpoint(self):
        ...
        self.assertIsNotNone(session_holder.dataFrameCache().getOrDefault(cached_remote_relation_id, None))
        ...
        def condition():
            ...
            self.assertIsNone(session_holder.dataFrameCache().getOrDefault(cached_remote_relation_id, None))
