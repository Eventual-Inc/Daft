...
class SparkConnectClientTestCase(unittest.TestCase):
    ...
    def test_user_agent_default(self):
        ...
        self.assertIsNotNone(mock.req, "ExecutePlan API was not called when expected")
        self.assertRegex(mock.req.client_type, r"^_SPARK_CONNECT_PYTHON spark/[^ ]+ os/[^ ]+ python/[^ ]+$")
    ...
    def test_channel_builder(self):
        ...
        client = SparkConnectClient(CustomChannelBuilder("sc://foo/"), use_reattachable_execute=False)
...
class SparkConnectClientReattachTestCase(unittest.TestCase):
    ...
    def test_fail_and_retry_during_execute(self):
        ...
        stub = self._stub_with([self.response, non_fatal], [self.response, self.response, self.finished])
        ite = ExecutePlanResponseReattachableIterator(self.request, stub, self.retrying, [])
        for b in ite:
            pass
    ...
    def test_fail_and_retry_during_reattach(self):
        ...
        stub = self._stub_with([self.response, non_fatal], [self.response, non_fatal, self.response, self.finished])
        ite = ExecutePlanResponseReattachableIterator(self.request, stub, self.retrying, [])
        for b in ite:
            pass
