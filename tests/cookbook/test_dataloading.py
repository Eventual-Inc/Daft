...
def test_glob_files_with_limit(batch_size, tmpdir):
    ...
    daft_df = daft.from_glob_path(path=[str(folder_a), str(folder_b)]).collect()
    assert 20 == len(daft_df)

    df = daft.from_glob_path(path=[str(folder_a), str(folder_b), f"{tmpdir}/ab", f"{tmpdir}/ac", f"{tmpdir}/a**/*"])

    daft_df = df.collect()
    assert 20 == len(daft_df)

    daft_df = df.limit(7).into_batches(batch_size).collect()
    assert 7 == len(daft_df)

    daft_df = df.limit(17).into_batches(batch_size).collect()
    assert 17 == len(daft_df)

    daft_df = df.limit(27).into_batches(batch_size).collect()
    assert 20 == len(daft_df)
