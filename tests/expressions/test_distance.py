...
def test_repr_distance():
    a = col("a")
    b = col("b")

    assert repr(a.cosine_distance(b)) == "cosine_distance(col(a), col(b))"
    assert repr(a.dot_product(b)) == "dot_product(col(a), col(b))"
    assert repr(a.euclidean_distance(b)) == "euclidean_distance(col(a), col(b))"


def _generate_valid_dtype_pairs_for_distance():
...
valid_distance_dtype_pairs = _generate_valid_dtype_pairs_for_distance()


@pytest.mark.parametrize("dtype_pair", valid_distance_dtype_pairs)
def test_dot_product(dtype_pair):
    ...
    res = df.select(col("source").dot_product(col("query"))).to_pydict()["source"]

    expected = [sum(x * y for x, y in zip(query, row)) for row in data["source"]]
...
@pytest.mark.parametrize("dtype_pair", valid_distance_dtype_pairs)
def test_cosine_distance(dtype_pair):
    dtype1, dtype2 = dtype_pair
    data = {
    ...
    res = df.select(col("source").cosine_distance(col("query"))).to_pydict()
    ...
    for actual, exp in zip(res["source"], expected):
        assert pytest.approx(exp) == pytest.approx(actual, abs=1e-5)
...
def test_pairwise_cosine_distance(dtype_pair):
    ...
    res = df.with_column(
        "distance",
        df["e1"].cast(dtype1).cosine_distance(df["e2"].cast(dtype2)),
    ).to_pydict()
...
def test_cosine_distance_dtype_not_fixed_size(data):
    ...
    with pytest.raises(
        ...
        df.with_column("distance", df["source"].cosine_distance(df["source"]))
...
def test_cosine_distance_dtype_size_mismatch():
    ...
    with pytest.raises(ValueError, match="Expected inputs to 'cosine_distance' to have the same size"):
        df.with_column("distance", df["source"].cosine_distance(df["query"]))
...
@pytest.mark.parametrize("dtype_pair", valid_distance_dtype_pairs)
def test_euclidean_distance(dtype_pair):
    dtype1, dtype2 = dtype_pair
    data = {
        "e1": [[1, 2, 3], [1, 2, 3]],
        "e2": [[1, 2, 3], [2, 3, 4]],
    ...
    df = daft.from_pydict(data)
    df = df.with_column("e1", col("e1").cast(dtype1))
    df = df.with_column("e2", col("e2").cast(dtype2))

    res = df.select(col("e1").euclidean_distance(col("e2"))).to_pydict()["e1"]

    expected = [0.0, math.sqrt(3)]
