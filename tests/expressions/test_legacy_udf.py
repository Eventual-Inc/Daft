...
def test_udf_with_runtime_env():
    ...
    with pytest.raises(
        ValueError,
        match="The `concurrency` must also be set when setting the `runtime_env` option.",
    ):
    ...
    with pytest.raises(
        ValueError,
        match="The `runtime_env` of UDF currently only supports configuring conda env, but got 'working_dir'",
    ):
