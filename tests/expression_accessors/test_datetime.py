...
def test_datetime_year():
    ...
    df = df.with_column("year", col("dt").datetime.year())
...
def test_datetime_month():
    data = {"dt": [datetime.date(1990 + i, i, i) for i in range(1, 13)]}
    df = DataFrame.from_pydict(data)
    df = df.with_column("month", col("dt").datetime.month())
    pd_df = pd.DataFrame.from_dict(data)
...
def test_datetime_day():
    data = {"dt": [datetime.date(1990 + i, i, i) for i in range(1, 13)]}
    df = DataFrame.from_pydict(data)
    df = df.with_column("day", col("dt").datetime.day())
    pd_df = pd.DataFrame.from_dict(data)
...
def test_datetime_day_of_week():
    data = {"dt": [datetime.date(1990 + i, i, i) for i in range(1, 13)]}
    df = DataFrame.from_pydict(data)
    df = df.with_column("day_of_week", col("dt").datetime.day_of_week())
    pd_df = pd.DataFrame.from_dict(data)
