...
def test_string_contains():
    ...
    df = df.with_column("contains_1", col("id").string.contains("1"))
...
def test_string_endswith():
    data = {"id": [str(i) for i in range(20)]}
    df = DataFrame.from_pydict(data)
    df = df.with_column("ends_with_1", col("id").string.endswith("1"))
    pd_df = pd.DataFrame.from_dict(data)
...
def test_string_endswith():
    data = {"id": [str(i) for i in range(20)]}
    df = DataFrame.from_pydict(data)
    df = df.with_column("starts_with_1", col("id").string.startswith("1"))
    pd_df = pd.DataFrame.from_dict(data)
...
def test_string_length():
    data = {"id": [str(i) for i in range(20)]}
    df = DataFrame.from_pydict(data)
    df = df.with_column("str_len", col("id").string.length())
    pd_df = pd.DataFrame.from_dict(data)
