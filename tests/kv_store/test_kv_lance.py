...
class TestEndToEndWorkflows:
    ...
    def test_error_handling_workflow(self, lance_manager):
        ...
        # Try accessing non-existent row IDs
        ...
        try:
            # Try accessing large amount of data
            large_batch = sparse_ds.take(list(range(1000)))  # Exceeds dataset size
            result = large_batch.to_pandas()
            # If successful, verify returned data amount
            assert len(result) <= sparse_ds.count_rows()
        except Exception:
            # If failed, this is also expected behavior
            pass
