...
class TestMultiSourceKVStore:
    ...
    def test_kv_batch_get_with_source_parameter(self):
        ...
        df_custom_batch = df.with_column(
            "features", 
            kv_batch_get("item_ids", source="lance_features", batch_size=500)
        )
    ...
    def test_multiple_sources_in_single_query(self):
        ...
        df_multi = (df
                   .with_column("embeddings", kv_get("item_id"))  # Default source
    ...
    def test_source_parameter_with_expression_column(self):
        ...
        df_batch_expr = df_batch.with_column(
            "embeddings", 
            kv_batch_get(df_batch["item_ids"], source="lance_embeddings")
        )
    ...
    def test_source_parameter_with_columns_and_error_handling(self):
        ...
        df_columns = df.with_column(
            "metadata", 
            kv_get("item_id", source="lance_metadata", columns=["col1", "col2"])
        )
        assert df_columns is not None
        
        # Test with on_error parameter
        df_error_null = df.with_column(
            "features",
            kv_get("item_id", source="lance_features", on_error="null")
        )
        ...
        df_both = df.with_column(
            "embeddings",
            kv_get("item_id", source="lance_embeddings", columns=["embedding"], on_error="raise")
        )
