...
@pytest.mark.parametrize(
    ...
    [
        ...
        (bool | None, dt.bool()),
        (str | None, dt.string()),
        (bytes | None, dt.binary()),
        (int | None, dt.int64()),
        (float | None, dt.float64()),
        (float | bool | None, dt.python()),
        (float | bool, dt.python()),
        (bool | int, dt.python()),
        (float | int | None, dt.python()),
        (datetime.datetime | None, dt.timestamp(TimeUnit.us())),
        (datetime.date | None, dt.date()),
        (datetime.time | None, dt.time(TimeUnit.us())),
        (datetime.timedelta | None, dt.duration(TimeUnit.us())),
        (list | None, dt.list(dt.python())),
        (list[str] | None, dt.list(dt.string())),
        (list[list] | None, dt.list(dt.list(dt.python()))),
        (list[list[str]] | None, dt.list(dt.list(dt.string()))),
        (
        ...
        *([] if tensorflow is None else [(tensorflow.Tensor, dt.tensor(dt.python()))]),
        ...
        (PIL.Image.Image | None, dt.image()),
        (Series | None, dt.list(dt.python())),
        (File | None, dt.file(MediaType.unknown())),
        (VideoFile | None, dt.file(MediaType.video())),
        (object, dt.python()),
        # Pydantic models
