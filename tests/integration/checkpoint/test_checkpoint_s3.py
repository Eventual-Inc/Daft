...
@pytest.mark.xfail(
    reason="DF-1918: checkpoint() on empty task errors — store needs to track in-flight writes",
    strict=True,
)
...
def test_checkpoint_s3_filtered_out_keys_are_checkpointed(minio_io_config):
    ...
    This is the fix for DF-1779: PushDownFilter must skip checkpointed sources
    so SCKO sees every row and checkpoints every source key.
    """
    with minio_create_bucket(minio_io_config) as bucket:
        input_path = f"s3://{bucket}/input"
        ckpt_prefix = f"s3://{bucket}/checkpoints"
...
def test_checkpoint_s3_rejects_concat_with_checkpoint(minio_io_config):
    """Reject concat/union pipelines with a checkpointed source (see DF-1921).
