...
def test_read_parquet_from_s3_with_include_file_path_column(minio_io_config):
    ...
    with minio_create_bucket(minio_io_config, bucket_name=bucket_name):
        ...
        read_back = daft.read_parquet(file_path, io_config=minio_io_config, file_path_column="path")
        assert read_back.to_pydict()["a"] == data["a"]
        assert read_back.to_pydict()["b"] == data["b"]
        assert read_back.to_pydict()["path"] == [file_path] * 3
...
def test_read_multi_parquet_from_s3_with_include_file_path_column(minio_io_config):
    ...
    with minio_create_bucket(minio_io_config, bucket_name=bucket_name):
        ...
        file_paths = [f"s3://{path}" for path in file_paths]
        read_back = daft.read_parquet(file_paths, io_config=minio_io_config, file_path_column="path")
        assert read_back.to_pydict()["a"] == data["a"]
        assert read_back.to_pydict()["b"] == data["b"]
        assert read_back.to_pydict()["path"] == file_paths
...
def test_read_csv_from_s3_with_include_file_path_column(minio_io_config):
    ...
    with minio_create_bucket(minio_io_config, bucket_name=bucket_name):
        ...
        assert len(file_paths) == 1
        file_path = f"s3://{file_paths[0]}"
        read_back = daft.read_csv(file_path, io_config=minio_io_config, file_path_column="path")
        assert read_back.to_pydict()["a"] == data["a"]
        assert read_back.to_pydict()["b"] == data["b"]
        assert read_back.to_pydict()["path"] == [file_path] * 3
...
def test_read_multi_csv_from_s3_with_include_file_path_column(minio_io_config):
    ...
    with minio_create_bucket(minio_io_config, bucket_name=bucket_name):
        file_paths = (
            daft.from_pydict(data)
            .into_partitions(3)
            .write_csv(f"s3://{bucket_name}", io_config=minio_io_config)
            .to_pydict()["path"]
        )
        assert len(file_paths) == 3
        file_paths = [f"s3://{path}" for path in file_paths]
        read_back = daft.read_csv(file_paths, io_config=minio_io_config, file_path_column="path")
        assert read_back.to_pydict()["a"] == data["a"]
        assert read_back.to_pydict()["b"] == data["b"]
        assert read_back.to_pydict()["path"] == file_paths
