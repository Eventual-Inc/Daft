...
def test_fixed_size_binary_concat(
    ...
    result = table.eval_expression_list([daft.functions.concat(col("a"), col("b"))])
...
def test_fixed_size_binary_concat_broadcast(
    ...
    result = table.eval_expression_list([daft.functions.concat(col("a"), lit(literal))])
    assert result.to_pydict() == {"a": expected_result}
    # Result should be Binary when using literals
    assert result.schema()["a"].dtype == DataType.binary()

    # Test left-side broadcasting
    result = table.eval_expression_list([daft.functions.concat(lit(literal), col("a"))])
...
def test_fixed_size_binary_concat_with_binary() -> None:
    ...
    result = table.eval_expression_list([daft.functions.concat(col("b"), col("a"))])
...
def test_fixed_size_binary_concat_with_literals() -> None:
    ...
    # Test with literal
    result = table.eval_expression_list([daft.functions.concat(col("a"), lit(b"xyz"))])
    assert result.to_pydict() == {"a": [b"abcxyz", b"defxyz", None]}
    # Result should be Binary when using literals
    assert result.schema()["a"].dtype == DataType.binary()

    # Test with null literal
    result = table.eval_expression_list([daft.functions.concat(col("a"), lit(None))])
...
def test_fixed_size_binary_concat_errors() -> None:
    ...
    with pytest.raises(Exception, match="Cannot infer supertypes for addition on types: Binary\[3\], Int64"):
        table.eval_expression_list([daft.functions.concat(col("a"), col("b"))])
