...
def test_table_size_bytes(TableCls) -> None:

    data = TableCls.from_pydict({"a": [1, 2, 3, 4, None], "b": [False, True, False, True, None]}).eval_expression_list(
...
@pytest.mark.parametrize("TableCls", [Table, MicroPartition])
def test_table_size_bytes_pyobj(TableCls, length) -> None:
    import pickle

    size_per_obj = len(pickle.dumps(object()))
    data = TableCls.from_pydict({"a": [object()] * length})
