...
def test_utf8_substr_zero_length() -> None:
    # Test various combinations of zero length substrings
    table = MicroPartition.from_pydict(
        {
            "col": ["hello", "Hello☃World", "😉test", "test🌈", "☃😉🌈"],
            "start": [0, 2, 4, 1, 3],
            "length": [0, 0, 0, 0, 0],  # All zero lengths
            "length_mixed": [0, 1, 0, 2, 0],  # Mix of zero and non-zero lengths
        }
    )

    # Test with scalar zero length
    result = table.eval_expression_list([col("col").str.substr(0, 0)])
    assert result.to_pydict() == {"col": [None, None, None, None, None]}

    # Test with column start and scalar zero length
    result = table.eval_expression_list([col("col").str.substr(col("start"), 0)])
    assert result.to_pydict() == {"col": [None, None, None, None, None]}

    # Test with scalar start and column zero length
    result = table.eval_expression_list([col("col").str.substr(1, col("length"))])
    assert result.to_pydict() == {"col": [None, None, None, None, None]}

    # Test with both column start and zero length
    result = table.eval_expression_list([col("col").str.substr(col("start"), col("length"))])
    assert result.to_pydict() == {"col": [None, None, None, None, None]}

    # Test with mixed zero and non-zero lengths
    result = table.eval_expression_list([col("col").str.substr(col("start"), col("length_mixed"))])
    assert result.to_pydict() == {
        "col": [
            None,  # length 0
            "l",  # length 1, start 2
    ...
    # Test zero length at string boundaries
    result = table.eval_expression_list(
        [
            col("col").str.substr(0, 0).alias("start"),  # At start
            col("col").str.substr(col("col").str.length(), 0).alias("end"),  # At end
            col("col")
            .str.substr(col("col").str.length().cast(DataType.int32()) - 1, 0)
            .alias("before_end"),  # Before end
        ]
    )
    assert result.to_pydict() == {
        "start": [None, None, None, None, None],
        "end": [None, None, None, None, None],
        "before_end": [None, None, None, None, None],
    }
