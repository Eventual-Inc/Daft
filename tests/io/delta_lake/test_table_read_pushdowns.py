...
def test_deltalake_read_predicate_pushdown_on_data(local_deltalake_table):
    path, tables = local_deltalake_table
    df = daft.read_delta_lake(str(path))
    df = df.where(df["a"] == 2)
    assert df.schema() == Schema.from_pyarrow_schema(deltalake.DeltaTable(path).schema().to_pyarrow())
    assert_pyarrow_tables_equal(df.to_arrow(), pa.concat_tables([table.filter(pc.field("a") == 2) for table in tables]))


def test_deltalake_read_predicate_pushdown_on_part(local_deltalake_table, partition_generator):
    path, tables = local_deltalake_table
    df = daft.read_delta_lake(str(path))
    part_idx = 2
    ...
    assert_pyarrow_tables_equal(
        df.to_arrow(), pa.concat_tables([table.filter(pc.field("part_idx") == part_value) for table in tables])
    )


def test_deltalake_read_predicate_pushdown_on_part_non_eq(local_deltalake_table, partition_generator):
    path, tables = local_deltalake_table
    df = daft.read_delta_lake(str(path))
    part_idx = 3
    partition_generator, _ = partition_generator
...
def test_deltalake_read_predicate_pushdown_on_part_and_data(local_deltalake_table, partition_generator):
    path, tables = local_deltalake_table
    df = daft.read_delta_lake(str(path))
    part_idx = 2
    partition_generator, _ = partition_generator
...
def test_deltalake_read_predicate_pushdown_on_part_and_data_same_clause(local_deltalake_table, partition_generator):
    path, tables = local_deltalake_table
    df = daft.read_delta_lake(str(path))
...
def test_deltalake_read_predicate_pushdown_on_part_empty(local_deltalake_table, partition_generator, num_partitions):
    partition_generator, _ = partition_generator
    path, tables = local_deltalake_table
    df = daft.read_delta_lake(str(path))
