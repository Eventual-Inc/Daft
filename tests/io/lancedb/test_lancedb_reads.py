...
def test_lancedb_read_limit_large_dataset(large_lance_dataset_path, limit_size, expected_scan_tasks):
    ...
    df = daft.read_lance(uri=large_lance_dataset_path)
...
def test_lancedb_read_parallelism_fragment_merging(large_lance_dataset_path, use_native_reader):
    """Test parallelism parameter reduces scan tasks by merging fragments."""
    df = daft.read_lance(uri=large_lance_dataset_path, fragment_group_size=3, use_native_reader=use_native_reader)
    result = df.to_pydict()
