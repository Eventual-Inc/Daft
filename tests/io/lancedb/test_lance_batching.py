...
def test_accumulate_small_micropartitions(schema, tmp_path):
    ...
    with patch("daft_lance.lance_data_sink.lance", fake):
        ...
        # 0.5.0 resolves the target table in start(); write() asserts it has run.
        sink.start()
...
def test_accumulates_under_default_threshold(schema, tmp_path):
    """Without an explicit threshold, daft-lance buffers against its own large default.

    Accumulation is unconditional since daft-lance 0.5.0: ``max_rows_per_file`` always
    has a value, so small micropartitions coalesce instead of becoming one fragment each.
    """
    fake = FakeLanceModule()._bind()
