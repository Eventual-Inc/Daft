...
def input_df():
    return daft.read_lance(uri="/tmp/daft/lance", use_native_reader=False)


def test_debug(input_df):
    print(input_df.filter("id > 100").count_rows())
    # print(daft.read_parquet("/tmp/daft/parquet").filter("id > 100").count_rows())
