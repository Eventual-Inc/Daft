...
def test_lancedb_roundtrip(lance_dataset_path):
    ...
    written_pa_schema = df_loaded.schema().to_arrow()
    written_metadata = written_pa_schema.metadata

    assert b"lance-encoding:compression" in written_metadata
    assert written_metadata[b"lance-encoding:compression"] == b"zstd"
