...
class TestDurationRoundtripFix:
    ...
    def test_duration_edge_cases(self, tmp_path):
        ...
        before = daft.from_arrow(pa.table({
            "zero_durations": pa.array(zero_data, type=pa.duration("ms"))
        }))
    ...
    def test_duration_large_values(self, tmp_path):
        ...
        before = daft.from_arrow(pa.table({
            "large_durations": pa.array(large_data, type=pa.duration("ms"))
        }))
    ...
    def test_duration_negative_values(self, tmp_path):
        ...
        before = daft.from_arrow(pa.table({
            "negative_durations": pa.array(negative_data, type=pa.duration("ms"))
        }))
    ...
    def test_duration_all_time_units(self, tmp_path, time_unit):
        ...
        before = daft.from_arrow(pa.table({
            f"duration_{time_unit}": pa.array(data, type=pa.duration(time_unit))
        }))
    ...
    def test_duration_in_complex_schema(self, tmp_path):
        ...
        expected_struct_dtype = DataType.struct({
            "duration_field": DataType.duration(TimeUnit.ms()),
    ...
    def test_regression_duration_not_int64(self, tmp_path):
        ...
        before = daft.from_arrow(pa.table({
            "duration_col": pa.array(duration_data, type=pa.duration("ms"))
        }))
