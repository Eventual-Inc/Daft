...
def test_jsonl_split_local_equals_unsplit(tmp_path):
    ...
    with daft.context.execution_config_ctx(scan_tasks_min_size_bytes=1, scan_tasks_max_size_bytes=128 * 1024):
...
def test_local_multiple_jsonl_files_partitions(tmp_path):
    ...
    with daft.context.execution_config_ctx(max_sources_per_scan_task=1):
        df2 = daft.read_json(paths)
        assert df2.num_partitions() == len(paths) == 5

    with daft.context.execution_config_ctx(max_sources_per_scan_task=3):
    ...
    with daft.context.execution_config_ctx(
        scan_tasks_min_size_bytes=1, scan_tasks_max_size_bytes=128 * 1024, max_sources_per_scan_task=1
    ):
...
def test_json_single_not_split(tmp_path):
    ...
    with daft.context.execution_config_ctx(scan_tasks_min_size_bytes=1, scan_tasks_max_size_bytes=4):
...
def test_jsonl_gzip_not_split(tmp_path):
    ...
    with daft.context.execution_config_ctx(scan_tasks_min_size_bytes=1, scan_tasks_max_size_bytes=128):
