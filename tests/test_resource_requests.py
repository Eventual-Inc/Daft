...
@pytest.mark.skipif(get_daft_settings().runner_config.name != "py", reason="requires PyRunner to be in use")
...
@pytest.mark.skipif(get_daft_settings().runner_config.name != "ray", reason="requires RayRunner to be in use")
@pytest.mark.skipif(no_gpu_available(), reason="requires GPUs to be available")
