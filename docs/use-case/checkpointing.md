...
Checkpointing helps Daft pipelines avoid re-processing rows that have already been processed in the event of a failure.

Daft pipelines that do expensive per-row work like LLM calls, embeddings, model inference, FFMPEG transforms, or third-party API calls can run for hours and incur significant cost. When they fail partway through, or get rerun for non-crash reasons (a bad UDF, bad config, bad input data), a restart from scratch is wasteful. Checkpointing tracks which inputs have been processed and skips them on rerun.

## Checkpointing Guarantees

Checkpointing offers two guarantees:

1. Upon rerunning a pipeline, inputs that were processed in a previous run are filtered out.

2. Idempotent commits: For Iceberg and Delta Lake writes, destination tables will resolve to the same state regardless if the job ran once or 15 times.

The first guarantee applies to all write methods that support checkpointing, but the second guarantee is necessarily limited to transactional table formats like Iceberg and Delta Lake since it takes advantage of the underlying catalogs. No duplicate snapshots, no double-committed rows. The connector pages cover which sinks support this and the per-sink details.
...
Imagine you are running a distributed pipeline that embeds millions of markdown files:
...
=== "🐍 Python"
    ...
    df.write_iceberg(
        table,
        checkpoint=daft.IdempotentCommit(store, idempotence_key="markdown-embeddings-2026-05-21"),
    )
    ```

On a rerun with the same checkpoint store and idempotence key, Daft skips inputs already recorded in the store and avoids landing a duplicate Iceberg snapshot.
