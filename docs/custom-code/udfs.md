...
!!! warning "New UDFs"
    Daft now offers a new UDF interface via the `@daft.func` and `@daft.cls` decorators. This new API is currently in active development and provides a cleaner interface for many use cases, with features like type inference, async functions, and generator functions.
    ...
    Check out the docs for the [new UDFs](func.md) for more details.
