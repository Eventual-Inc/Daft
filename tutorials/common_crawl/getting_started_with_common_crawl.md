...
```python
!pip install uv && uv pip install "daft[sentence-transformers]" torch spacy
```
...
import torch
import daft
from daft import col, Series, DataType
import numpy as np
import spacy
import spacy.cli
```
...
Let's take a look at the first few records to see what they contain. Run the below cell. If the contents of a cell are too long, you can **double click** on a cell to get a pop-out that shows more details.
...
class ChunkingUDF:
    ...
    @daft.method.batch(return_dtype=list[TextChunk])
    def __call__(self, text_col: Series) -> list[list[TextChunk]]:
        n_truncated_spacy = 0
        n_truncated_sentence_transformer = 0
        results: list[list[TextChunk]] = []
...
df_chunk = (
    df_lang
      .with_column("chunks", chunker(col("warc_content")))
      # we need to "unroll" our sentence-chunked text!
      .explode("chunks")
...
Now that we have nice sentence-chunked text, we can produce embeddings! Daft makes it very easy to run models on your data. We can make use of the [`embed_text`](https://docs.daft.ai/en/stable/api/functions/embed_text/#daftfunctionsembed_text) function to use a locally-running model (from the `sentence-transformers` library) to compute embeddings.
...
df = (
    daft.datasets.common_crawl(
      ...
      segment=None,
      content="text",
      num_files=1,   # INCREASE THIS NUMBER TO RUN ON MORE CRAWL FILES
...
We can take a peek to see what this looks like!
