...
/// A trait describing an array that can be split at a given offset.
...
pub trait MutableArray: std::fmt::Debug + Send + Sync {
    ...
    // boxes `self`, then converts the box into an `Arc`. Implementers may wish
