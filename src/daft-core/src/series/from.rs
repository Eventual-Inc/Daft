...
impl TryFrom<(&str, Box<dyn arrow2::array::Array>)> for Series {
    ...
    fn try_from(item: (&str, Box<dyn arrow2::array::Array>)) -> DaftResult<Self> {
        let (name, array) = item;
        let self_arrow_type = array.data_type(); // arrow2 Decimal128(..), array: PrimitiveArray<i128>
        ...
        if dtype.is_logical() {
            ...
            let res = with_match_daft_logical_primitive_types!(dtype, |$P| {
                use arrow2::array::Array;
                let physical_arrow_array = array
                    .as_any()
