...
use crate::{
    ...
    datatypes::{DaftArrowBackedType, DataType, FileArray, FixedSizeBinaryArray, IntervalArray},
    lit::Literal,
...
impl SeriesLike for ArrayWrapper<IntervalArray> {
    fn into_series(&self) -> Series {
        self.0.clone().into_series()
    }
    fn to_arrow(&self) -> Box<dyn arrow2::array::Array> {
...
impl SeriesLike for ArrayWrapper<FileArray> {
    fn into_series(&self) -> Series {
        self.0.clone().into_series()
    }
    fn to_arrow(&self) -> Box<dyn arrow2::array::Array> {
