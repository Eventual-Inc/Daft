...
use crate::{
    ...
    datatypes::FileArray,
    file::{
        MediaTypeAudio, MediaTypeHdf5, MediaTypeImage, MediaTypeMcap, MediaTypeUnknown,
        MediaTypeVideo,
    },
    prelude::*,
};
    ...
    let s = match downcasted.clone() {
        ...
        DataType::File(media_type) => {
            ...
            match media_type {
                ...
                daft_schema::media_type::MediaType::Mcap => {
                    FileArray::<MediaTypeMcap>::new_from_file_references("literal", iter)?
                        .into_series()
                }
            }
        }
