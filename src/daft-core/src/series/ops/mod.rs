...
macro_rules! py_binary_op_utilfn {
    ($lhs:expr, $rhs:expr, $pyoperator:expr, $utilfn:expr) => {{
        ...
        let result_series: Series = Python::with_gil(|py| -> PyResult<PySeries> {
            let py_operator = PyModule::import_bound(py, "operator")?.getattr(($pyoperator))?;

            let result_pylist = PyModule::import_bound(py, "daft.utils")?
                .getattr(($utilfn))?
            ...
            PyModule::import_bound(py, "daft.series")?
                .getattr("Series")?
                .getattr("from_pylist")?
                .call1((result_pylist, lhs.name(), ("disallow")))?
                .getattr("_series")?
                .extract()
        })?
        .into();
...
pub(super) fn py_membership_op_utilfn(lhs: &Series, rhs: &Series) -> DaftResult<Series> {
    ...
    let result_series: Series = Python::with_gil(|py| -> PyResult<PySeries> {
        let result_pylist = PyModule::import_bound(py, "daft.utils")?
            .getattr("python_list_membership_check")?
            .call1((left_pylist, right_pylist))?;

        PyModule::import_bound(py, "daft.series")?
            .getattr("Series")?
            .getattr("from_pylist")?
            .call1((result_pylist, lhs_casted.name(), ("disallow")))?
            .getattr("_series")?
            .extract()
    })?
    .into();
...
pub(super) fn py_between_op_utilfn(
    ...
    let result_series: Series = Python::with_gil(|py| -> PyResult<PySeries> {
        let result_pylist = PyModule::import_bound(py, "daft.utils")?
            .getattr("python_list_between_check")?
            .call1((value_pylist, lower_pylist, upper_pylist))?;

        PyModule::import_bound(py, "daft.series")?
            .getattr("Series")?
            .getattr("from_pylist")?
            .call1((result_pylist, value_casted.name(), ("disallow")))?
            .getattr("_series")?
            .extract()
    })?
    .into();
