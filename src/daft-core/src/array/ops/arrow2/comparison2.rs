...
pub fn build_dyn_multi_array_is_equal(
    ...
    for field in schema.fields.values() {
        fn_list.push(build_dyn_is_equal(&field.dtype, &field.dtype, nulls_equal, nan_equal)?);
    }
    let combined_fn = Box::new(move |left: &[&dyn Array], right: &[&dyn Array], i: usize, j: usize| -> bool {
