...
impl IntoGroups for Float32Array {
    fn make_groups(&self) -> DaftResult<super::GroupIndicesPair> {
        if self.null_count() > 0 {
            make_groups(self.into_iter().map(move |f| {
                f.map(|v| {
                    match v.is_nan() {
                        true => f32::NAN,
                        false => *v,
            ...
            make_groups(self.values().iter().map(move |f| {
                match f.is_nan() {
                    true => f32::NAN,
                    false => *f,
                }
...
impl IntoUniqueIdxs for Float32Array {
    fn make_unique_idxs(&self) -> DaftResult<super::VecIndices> {
        if self.null_count() > 0 {
            make_unique_idxs(self.into_iter().map(move |f| {
                f.map(|v| {
                    match v.is_nan() {
                        true => f32::NAN,
                        false => *v,
            ...
            make_unique_idxs(self.values().iter().map(move |f| {
                match f.is_nan() {
                    true => f32::NAN,
                    false => *f,
                }
...
impl IntoGroups for Utf8Array {
    fn make_groups(&self) -> DaftResult<super::GroupIndicesPair> {
        if self.null_count() > 0 {
            ...
            make_groups(self.clone().into_values().into_iter())
        }
    }
}
...
impl IntoUniqueIdxs for Utf8Array {
    fn make_unique_idxs(&self) -> DaftResult<super::VecIndices> {
        if self.null_count() > 0 {
            ...
            make_unique_idxs(self.clone().into_values().into_iter())
        }
    }
}
