...
mod tests {
    ...
    fn test_fixed_shape_sparse_datatype() -> DaftResult<()> {
        ...
        let indices_minimal_dtype = [DataType::UInt8, DataType::UInt16, DataType::UInt32, DataType::UInt64];

        for (n_elements, minimal_dtype) in element_counts.iter().zip(indices_minimal_dtype.iter()) {
            let dtype = DataType::FixedShapeSparseTensor(Box::new(DataType::Float32), vec![*n_elements]);
            ...
            if let DataType::Struct(fields) = physical_dtype {
                ...
                assert_eq!(indices_field.dtype, DataType::List(Box::new(minimal_dtype.clone())));
            } else {
