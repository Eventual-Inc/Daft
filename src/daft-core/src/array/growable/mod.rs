...
macro_rules! series_to_arrow_array {
    ($series:ident, $daft_type:path) => {
        $series
            .iter()
            .map(|s| s.downcast::<DataArray<$daft_type>>().unwrap().as_arrow())
            .collect::<Vec<_>>()
    };
}

pub fn make_growable(
    name: String,
    dtype: &DataType,
    series: &[&Series],
    capacity: usize,
    ...
    match dtype {
        ...
        // DataType::Float16 => Box::new(ArrowFloat16Growable{ dtype:name,  dtype.clone(), arrow2_growable: arrow2::array::growable::GrowablePrimitive::<Float16>::new(), _phantom: PhantomData}),
