...
use super::{IntoGroups, IntoUniqueIdxs};
...
#[inline(never)]
fn make_unique_idxs<T>(iter: impl Iterator<Item = T>) -> DaftResult<super::Indices>
where
    T: Hash,
    T: Eq,
{
...
macro_rules! impl_into_groups_primitive_array {
    ($T:ty) => {
        ...
        impl IntoUniqueIdxs for DataArray<$T> {
            fn make_unique_idxs(&self) -> DaftResult<super::Indices> {
                if self.null_count() > 0 {
