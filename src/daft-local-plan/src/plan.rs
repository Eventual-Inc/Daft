...
use daft_dsl::{
    ...
    functions::python::{get_resource_request, UDFProperties},
    ...
    pub fn with_new_children(&self, children: &[Arc<Self>]) -> Arc<Self> {
        match children {
            [new_child] => match self {
                ...
                Self::UDFProject(UDFProject { expr, udf_properties, passthrough_columns, schema, .. }) => Self::udf_project(new_child.clone(), expr.clone(), udf_properties.clone(), passthrough_columns.clone(), schema.clone(), StatsState::NotMaterialized),
