    ...
    pub fn limit(
        ...
        if offset.is_some_and(|o| o > limit) {
            return Err(DaftError::ValueError(format!(
                "Input offset {} is greater than limit {}",
                offset.unwrap(),
                limit
            )));
        }
    ...
    pub fn with_new_children(&self, children: &[Arc<Self>]) -> Arc<Self> {
        match children {
            [new_child] => match self {
                ...
                Self::Limit(Limit { limit, offset, .. }) => Self::limit(new_child.clone(), *limit, *offset, StatsState::NotMaterialized).unwrap(),
                ...
                Self::TopN(TopN { sort_by, descending, nulls_first, limit, offset, .. }) => Self::top_n(new_child.clone(), sort_by.clone(), descending.clone(), nulls_first.clone(), *limit, *offset, StatsState::NotMaterialized).unwrap(),
