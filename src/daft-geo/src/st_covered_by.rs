...
impl ScalarUDF for StCoveredBy {
    ...
    fn docstring(&self) -> &'static str {
        "Returns true if geometry A is covered by geometry B (no point of A lies outside B; includes boundary)."
    }
}

#[must_use]
pub fn st_covered_by(geom_a: ExprRef, geom_b: ExprRef) -> ExprRef {
    ScalarFn::builtin(StCoveredBy, vec![geom_a, geom_b]).into()
}
