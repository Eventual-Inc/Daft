...
#[typetag::serde]
impl ScalarUDF for StCrosses {
    fn name(&self) -> &'static str { "st_crosses" }
    fn call(&self, inputs: FunctionArgs<Series>, _ctx: &daft_dsl::functions::scalar::EvalContext) -> DaftResult<Series> {
        binary_geom_to_bool(inputs.required(0)?, inputs.required(1)?, self.name(),
            |a, b| relate_pred(a, b, RelatePred::Crosses))
    }
    ...
    fn docstring(&self) -> &'static str { "Returns true if A and B cross (interiors intersect with lower dimension than the inputs)." }
}

#[must_use]
pub fn st_crosses(geom_a: ExprRef, geom_b: ExprRef) -> ExprRef {
    ScalarFn::builtin(StCrosses, vec![geom_a, geom_b]).into()
}
