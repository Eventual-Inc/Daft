...
#[typetag::serde]
impl ScalarUDF for StOverlaps {
    fn name(&self) -> &'static str { "st_overlaps" }
    fn call(&self, inputs: FunctionArgs<Series>, _ctx: &daft_dsl::functions::scalar::EvalContext) -> DaftResult<Series> {
        binary_geom_to_bool(inputs.required(0)?, inputs.required(1)?, self.name(),
            |a, b| relate_pred(a, b, RelatePred::Overlaps))
    }
    ...
    fn docstring(&self) -> &'static str { "Returns true if A and B overlap (same dimension, interiors intersect, neither contains the other)." }
}

#[must_use]
pub fn st_overlaps(geom_a: ExprRef, geom_b: ExprRef) -> ExprRef {
    ScalarFn::builtin(StOverlaps, vec![geom_a, geom_b]).into()
}
