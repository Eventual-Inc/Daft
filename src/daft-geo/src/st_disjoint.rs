...
#[typetag::serde]
impl ScalarUDF for StDisjoint {
    fn name(&self) -> &'static str { "st_disjoint" }
    fn call(&self, inputs: FunctionArgs<Series>, _ctx: &daft_dsl::functions::scalar::EvalContext) -> DaftResult<Series> {
        binary_geom_to_bool(inputs.required(0)?, inputs.required(1)?, self.name(),
            |a, b| relate_pred(a, b, RelatePred::Disjoint))
    }
    ...
    fn docstring(&self) -> &'static str { "Returns true if A and B share no points." }
}

#[must_use]
pub fn st_disjoint(geom_a: ExprRef, geom_b: ExprRef) -> ExprRef {
    ScalarFn::builtin(StDisjoint, vec![geom_a, geom_b]).into()
}
