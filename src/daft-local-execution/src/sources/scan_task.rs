...
async fn forward_scan_task_stream(
    ...
    let stream_res = stream_scan_task(scan_task.clone(), io_stats, delete_map, maintain_order, chunk_size).await;
    let mut stream = match stream_res {
        Ok(s) => s,
        Err(e) => {
            match scan_task.file_format_config.as_ref() {
                FileFormatConfig::Parquet(ParquetSourceConfig { ignore_error: true, .. }) => {
                    log::warn!("Skipping unreadable/corrupt parquet file {}: {}", scan_task.sources.first().map(|s| s.get_path()).unwrap_or("UNKNOWN"), e);
                    return Ok(());
                }
                FileFormatConfig::Csv(CsvSourceConfig { ignore_error: true, .. }) => {
                    log::warn!("Skipping unreadable/corrupt csv file {}: {}", scan_task.sources.first().map(|s| s.get_path()).unwrap_or("UNKNOWN"), e);
                    return Ok(());
                }
                _ => return Err(e),
            }
...
async fn stream_scan_task(
    ...
    let mut table_stream = match scan_task.file_format_config.as_ref() {
        FileFormatConfig::Parquet(ParquetSourceConfig {
            ...
            ignore_error: parquet_ignore_error,
            ..
        }) => {
            ...
            table_stream = Box::pin(table_stream.filter_map(move |res| {
                ...
                async move {
                    match res {
                        Ok(tbl) => Some(Ok(tbl)),
                        Err(e) => {
                            log::warn!("Skipping unreadable/corrupt parquet file {}: {}", url_clone, e);
                            None
                        }
                    }
                }
