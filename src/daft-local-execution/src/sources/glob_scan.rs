    ...
    async fn get_data(
        ...
        let task = io_runtime
            .spawn(async move {
                ...
                let subtasks = glob_paths.iter().unique().map(|glob_path| {
                    ...
                    async move {
                        ...
                        while let Some(files_chunk) = stream.next().await {
                            ...
                            // Apply global limit before sending
                            let mut should_send = true;
                            let mut done = false;
                            let mut remaining = remaining_rows.lock().await;
                            if let Some(rows_left) = *remaining {
                                if rows_left == 0 {
                                    should_send = false;
                                    done = true;
                                } else {
                                    let new_rows_left = rows_left.saturating_sub(num_rows);
                                    *remaining = Some(new_rows_left);
                                    if new_rows_left == 0 {
                                        done = true;
                                    }
                                }
                            ...
                            if should_send && tx.send(Ok(partition)).await.is_err() {
                                break;
                            }
                ...
                // If no files were found across all paths, return an error
