...
impl RepartitionState {
    ...
    fn clear(&mut self, _num_partitions: usize) {
...
impl PipelineNode for RepartitionNode {
    ...
    fn start(
        ...
        let _task_spawner = ExecutionTaskSpawner::new(
            get_compute_runtime(),
            runtime_handle.memory_manager(),
            self.runtime_stats.clone(),
            info_span!("RepartitionNode::Execute"),
        );
...
mod tests {
    ...
    use super::RepartitionState;

    #[test]
    fn test_repartition_state_basic() {
        use daft_core::{prelude::*, series::Series};

        let mut state = RepartitionState::new(2);
        ...
        let create_mp = || {
            let array = Int64Array::from(("a", vec![1, 2, 3]));
            let series = array.into_series();
            let rb = daft_recordbatch::RecordBatch::new_with_size(schema.clone(), vec![series], 3)
                .unwrap();
            MicroPartition::new_loaded(schema.clone(), Arc::new(vec![rb]), None)
        };

        // Push data for partition 0 and 1
        state.push(vec![create_mp(), create_mp()]);
        assert!(!state.is_empty());

        // Emit partition 0
        let emitted = state.emit();
        assert!(emitted.is_some());
        let parts = emitted.unwrap();
        ...
        // Emit partition 2 (should be None)
        assert!(state.emit().is_none());
