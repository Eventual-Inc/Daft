...
use crate::{
    ...
    ExecutionRuntimeContext, NativeExecutor, PipelineCreationSnafu,
};
    ...
    pub(crate) fn physical_plan_to_pipeline(
        ...
        let out: Box<dyn PipelineNode> = match physical_plan {
            ...
            LocalPhysicalPlan::WindowPartitionOnly(WindowPartitionOnly {
                ...
                let input_node = self.physical_plan_to_pipeline(input, psets, cfg, ctx)?;
            ...
            LocalPhysicalPlan::WindowPartitionAndDynamicFrame(WindowPartitionAndDynamicFrame {
                ...
                let input_node = self.physical_plan_to_pipeline(input, psets, cfg, ctx)?;
                let window_partition_and_dynamic_frame_sink =
                    WindowPartitionAndDynamicFrameSink::new(
                        aggregations,
                        *min_periods,
                        aliases,
                        partition_by,
            ...
            LocalPhysicalPlan::Project(Project {
                ...
                let child_node = self.physical_plan_to_pipeline(input, psets, cfg, ctx)?;
            ...
            LocalPhysicalPlan::Concat(Concat {
                ...
                let left_child = self.physical_plan_to_pipeline(input, psets, cfg, ctx)?;
                let right_child = self.physical_plan_to_pipeline(other, psets, cfg, ctx)?;
                let sink = ConcatSink {};
            ...
            LocalPhysicalPlan::UnGroupedAggregate(UnGroupedAggregate {
                ...
                let child_node = self.physical_plan_to_pipeline(input, psets, cfg, ctx)?;
                let agg_sink =
                    AggregateSink::new(aggregations, input.schema()).with_context(|_| {
                        PipelineCreationSnafu {
                            plan_name: physical_plan.name(),
                        }
                    })?;
                BlockingSinkNode::new(Arc::new(agg_sink), child_node, stats_state.clone(), ctx)
                    .boxed()
            }
            LocalPhysicalPlan::HashAggregate(HashAggregate {
                ...
                let child_node = self.physical_plan_to_pipeline(input, psets, cfg, ctx)?;
                let agg_sink =
                    GroupedAggregateSink::new(aggregations, group_by, input.schema(), cfg)
                        .with_context(|_| PipelineCreationSnafu {
                            plan_name: physical_plan.name(),
                        })?;
                BlockingSinkNode::new(Arc::new(agg_sink), child_node, stats_state.clone(), ctx)
            ...
            LocalPhysicalPlan::Sort(Sort {
                ...
                let sort_sink =
                    SortSink::new(sort_by.clone(), descending.clone(), nulls_first.clone());
                let child_node = self.physical_plan_to_pipeline(input, psets, cfg, ctx)?;
                BlockingSinkNode::new(Arc::new(sort_sink), child_node, stats_state.clone(), ctx)
                    .boxed()
            }
            ...
            LocalPhysicalPlan::HashJoin(HashJoin {
                ...
                || -> DaftResult<_> {
                    ...
                    let build_child_node = self.physical_plan_to_pipeline(build_child, psets, cfg, ctx)?;
                    ...
                    let probe_child_node = self.physical_plan_to_pipeline(probe_child, psets, cfg, ctx)?;
            ...
            LocalPhysicalPlan::CrossJoin(CrossJoin {
                ...
                let stream_child_node =
                    self.physical_plan_to_pipeline(stream_child, psets, cfg, ctx)?;
                let collect_child_node =
                    self.physical_plan_to_pipeline(collect_child, psets, cfg, ctx)?;
