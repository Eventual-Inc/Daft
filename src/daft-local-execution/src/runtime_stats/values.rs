...
pub struct DefaultRuntimeStats {
    ...
    num_tasks: Counter,
    spill: SpillReporter,
    /// Latest reported in-memory buffer size for blocking sinks. The 200ms
    /// aggregation in `StatSnapshot::merge` takes the max across concurrent
    /// inputs, giving a high-water reading per tick.
...
impl DefaultRuntimeStats {
    ...
    /// (e.g. flight shuffle). Callers clone this `Arc` down to the I/O layer
    /// so it can record bytes/file events directly.
