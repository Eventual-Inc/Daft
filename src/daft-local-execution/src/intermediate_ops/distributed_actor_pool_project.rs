...
pub(crate) struct DistributedActorPoolProjectOperator {
    ...
    memory_request: u64,
    passthrough_cols: Vec<BoundExpr>, // Columns that no need to be passed to UDFActor for processing
    required_cols: Vec<usize>,        // UDF input columns
...
impl IntermediateOperator for DistributedActorPoolProjectOperator {
    ...
    fn execute(
        ...
        let passthrough_cols = self.passthrough_cols.clone();
            ...
            let fut = task_spawner.spawn_with_memory_request(
                ...
                async move {
                    ...
                    let pruned_input = Arc::new(MicroPartition::new_loaded(
                        Arc::new(Schema::new(
                            required_cols.iter().map(|idx| input_schema[*idx].clone()),
                        )),
                        Arc::new(
                            input
                                .clone()
                        ...
                        .zip(eval_output.record_batches().iter())
                    {
                        let passthrough_record =
                            input_record.eval_expression_list(passthrough_cols.as_slice())?;
