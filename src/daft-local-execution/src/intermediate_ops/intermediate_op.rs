...
impl<Op: IntermediateOperator + 'static> PipelineNode for IntermediateNode<Op> {
    ...
    fn start(
        ...
        // using the maintain_order parameter from operator, especially for spawn_workers,
        // all worker_receivers will be combined to a sender which consume data from each receiver one by one,
        // if the send block, will block other receivers.
