...
function TimelinePageInner() {
  ...
  useEffect(() => {
    ...
    es.onopen = () => {
      console.info("Connected to query SSE endpoint (timeline)");
    };
    es.onerror = event => {
    ...
    es.addEventListener("initial_state", event => {
      ...
      if (debug) console.log("[debug] initial_state (timeline)", data);
      setQuery(data);
    });
    es.addEventListener("query_info", event => {
      const data: QueryInfo = JSON.parse(event.data);
      if (debug) console.log("[debug] query_info (timeline)", data);
      setQuery(data);
    });
    es.addEventListener("operator_info", event => {
      const data: Record<number, OperatorInfo> = JSON.parse(event.data);
      if (debug) console.log("[debug] operator_info (timeline)", data);
      setQuery(prev => {
        if (!prev) return prev;
    ...
    return () => {
      console.info("Closing query SSE endpoint (timeline)");
      es.close();
    };
  }, [queryId, debug]);
