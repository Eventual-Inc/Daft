  ...
  return (
    <nav className="bg-background border-b border-zinc-700 flex items-center justify-between h-[50px]">
      ...
      <div className="flex items-center gap-5 px-6 h-full">
        ...
        <div className={`h-full border-(--daft-accent) ${pathName.startsWith(`/queries`) ? "border-b" : ""}`}>
          <Button
            asChild
            variant="ghost"
            className="h-full transition-colors rounded-none hover:bg-zinc-700"
          >
            <Link href="/queries" className="flex items-center gap-2 h-full px-4">
              <TableProperties size={16} />
              <span className="font-mono font-bold">All Queries</span>
              <Badge className="bg-(--daft-accent) h- min-w-5 rounded-full px-1.5 font-mono tabular-nums">{activeQueries.length}</Badge>
            </Link>
          </Button>
        </div>
