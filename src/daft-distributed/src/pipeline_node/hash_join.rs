...
use daft_logical_plan::{stats::StatsState, InMemoryInfo, JoinType};
...
impl HashJoinNode {
    ...
    async fn execution_loop(
        ...
        let left_materialized = left_repartition.materialize(scheduler_handle.clone());
        let left_transposed =
            RepartitionNode::transpose_materialized_outputs(left_materialized, self.num_partitions)
                .await?;

        // Same for the the right
        let right_materialized = right_repartition.materialize(scheduler_handle.clone());
