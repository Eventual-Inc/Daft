...
impl PipelineNodeImpl for PreShuffleMergeFlightNode {
    ...
    fn multiline_display(&self, _verbose: bool) -> Vec<String> {
        vec![format!("Pre-Shuffle Merge Flight")]
    }
    ...
    fn produce_tasks(
        ...
        let merge_execution = async move {
            self_merge
                .execute_merge(wrapped_input, task_id_counter, merge_tx, scheduler_handle)
                .await
        };
...
impl PreShuffleMergeFlightNode {
    /// Phase 2: Materialize wrapped child tasks (flight_gather_write), accumulate all
    /// (task_id, server_address) per worker, then create one merge task per worker.
    async fn execute_merge(
        ...
        let mut worker_entries: HashMap<WorkerId, Vec<(TaskID, String)>> = HashMap::new();
        ...
        while let Some(output) = materialized_stream.try_next().await? {
            ...
            worker_entries
                .entry(worker_id)
                .or_default()
                .push((task_id, server_address));
        }

        // Create one merge task per worker: flight_gather_write(flight_shuffle_read_with_cache_ids(...))
        for (worker_id, entries) in worker_entries {
            let mut server_cache_mapping: HashMap<String, Vec<TaskID>> = HashMap::new();
            for (cache_id, server) in entries {
            ...
            let builder = SwordfishTaskBuilder::new(merge_plan, self.as_ref()).with_strategy(Some(
