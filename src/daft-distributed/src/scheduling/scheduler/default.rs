...
impl<T: Task> DefaultScheduler<T> {
    ...
    fn needs_autoscaling(&self) -> bool {
        ...
        // Use total demand (pending + just-dispatched) to compute the ratio, since
        // schedule_tasks() drains pending tasks before this check runs. Without the
        // dispatched count, the ratio only reflects residual demand.
        ...
        tracing::debug!(
            ...
            dispatched = self.last_scheduled_count,
            total_demand,
            total_capacity,
            ratio,
            threshold = self.autoscaling_threshold,
            "Autoscaling ratio check"
        );
