...
use crate::{
    ...
    utils::channel::{create_oneshot_channel, OneshotReceiver, OneshotSender},
};
...
pub(crate) struct SwordfishTaskBuilder {
    ...
    context: HashMap<String, String>,
    // For automatic context creation
    node_context: Option<PipelineNodeContext>,
    // Pending node_ids to add when task_context is created during build
    pending_node_ids: Vec<NodeID>,
    // For notify tokens and cancellation
...
impl SwordfishTaskBuilder {
    ...
    /// - Merged scan_tasks from both builders (right takes precedence on conflicts)
    ...
    pub fn build(
        ...
        SubmittableTask::from_parts(task, cancel_token, self.notify_tokens)
    }
}
