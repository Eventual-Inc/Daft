...
use crate::scheduling::{
    dispatcher::SchedulableTask, scheduler::{DefaultScheduler, ScheduleResult, Scheduler}, task::{SchedulingStrategy, Task, TaskId}, tests::{MockPartition, MockTask, MockWorker}, worker::{Worker, WorkerId}
};

use super::MockTaskBuilder;

// Helper to create a schedulable task
fn create_schedulable_task(strategy: SchedulingStrategy, partition_ref: PartitionRef) -> SchedulableTask<MockTask> {
...
fn test_spread_scheduling() {
    ...
    let mut scheduler = Box::new(DefaultScheduler::new()) as Box<dyn Scheduler<MockTask, MockWorker>>;
...
fn test_soft_node_affinity_scheduling() {
    ...
    let tasks = vec![
        ...
        create_schedulable_task(SchedulingStrategy::NodeAffinity { 
    ...
    let scheduled_workers: Vec<String> = result.scheduled_tasks.iter()
        .map(|(id, _)| id.clone())
        .collect();
    
    // Count occurrences of each worker
    let worker1_count = scheduled_workers.iter().filter(|&id| id == "worker1").count();
    let worker3_count = scheduled_workers.iter().filter(|&id| id == "worker3").count();
...
fn test_mixed_scheduling_strategies() {
    ...
    assert!(result.scheduled_tasks.iter().any(|(id, task)| 
        id == "worker2" && matches!(task.strategy(), SchedulingStrategy::Spread)
    ));
    
    // Hard affinity task should go to worker1
    assert!(result.scheduled_tasks.iter().any(|(id, task)| 
        id == "worker1" && matches!(task.strategy(), SchedulingStrategy::NodeAffinity { node_id, soft } 
            if node_id == "worker1" && !soft)
    ));
    
    // Soft affinity task should go to worker2
    assert!(result.scheduled_tasks.iter().any(|(id, task)| 
        id == "worker2" && matches!(task.strategy(), SchedulingStrategy::NodeAffinity { node_id, soft } 
