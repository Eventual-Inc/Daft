    ...
    fn get_result(&mut self) -> impl Future<Output = TaskStatus> + Send + 'static {
        ...
        async move {
            match execute_swordfish_task_locally(plan, config, inputs, psets, task_id).await {
                Ok((partitions, stats)) => TaskStatus::Success {
                    result: TaskOutput::Materialized(MaterializedOutput::new(
                        partitions,
                        "local-worker".into(),
                        String::new(),
                        task_id,
...
async fn execute_swordfish_task_locally(
    ...
    _task_id: u32,
) -> DaftResult<(Vec<PartitionRef>, ExecutionStats)> {
    ...
    let (result_partitions, stats) =
        daft_local_execution::execute_local_plan(&plan, config, inputs).await?;

    // Convert output MicroPartitions into PartitionRefs
    let partition_refs: Vec<PartitionRef> = result_partitions
        .into_iter()
        .map(|mp| Arc::new(mp) as PartitionRef)
        .collect();

    Ok((partition_refs, stats))
}
