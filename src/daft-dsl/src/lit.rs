...
impl Hash for LiteralValue {
    fn hash<H: Hasher>(&self, state: &mut H) {
        match self {
            ...
            LiteralValue::Null => 1.hash(state),
            LiteralValue::Boolean(bool) => bool.hash(state),
            LiteralValue::Utf8(s) => s.hash(state),
            LiteralValue::Binary(arr) => arr.hash(state),
            LiteralValue::Int32(n) => n.hash(state),
            LiteralValue::UInt32(n) => n.hash(state),
            LiteralValue::Int64(n) => n.hash(state),
            LiteralValue::UInt64(n) => n.hash(state),
            LiteralValue::Date(n) => n.hash(state),
            LiteralValue::Time(n, tu) => {
                n.hash(state);
                tu.hash(state);
            }
            LiteralValue::Timestamp(n, tu, tz) => {
                n.hash(state);
                tu.hash(state);
                tz.hash(state);
            }
            // Wrap float64 in hashable newtype.
            LiteralValue::Float64(n) => FloatWrapper(*n).hash(state),
            LiteralValue::Decimal(n, precision, scale) => {
                n.hash(state);
            ...
            #[cfg(feature = "python")]
            LiteralValue::Python(py_obj) => py_obj.hash(state),
        }
    }
}
...
impl Display for LiteralValue {
    ...
    fn fmt(&self, f: &mut Formatter) -> Result {
        match self {
            ...
            LiteralValue::Utf8(val) => write!(f, "\"{val}\""),
            LiteralValue::Binary(val) => write!(f, "Binary[{}]", val.len()),
            LiteralValue::Int32(val) => write!(f, "{val}"),
            LiteralValue::UInt32(val) => write!(f, "{val}"),
            LiteralValue::Int64(val) => write!(f, "{val}"),
            LiteralValue::UInt64(val) => write!(f, "{val}"),
            LiteralValue::Date(val) => write!(f, "{}", display_date32(*val)),
            LiteralValue::Time(val, tu) => write!(f, "{}", display_time64(*val, tu)),
            LiteralValue::Timestamp(val, tu, tz) => {
                write!(f, "{}", display_timestamp(*val, tu, tz))
            }
            LiteralValue::Float64(val) => write!(f, "{val:.1}"),
            LiteralValue::Decimal(val, precision, scale) => {
                write!(f, "{}", display_decimal128(*val, *precision, *scale))
            }
            LiteralValue::Series(series) => write!(f, "{}", display_series_literal(series)),
            #[cfg(feature = "python")]
            LiteralValue::Python(pyobj) => write!(f, "PyObject({})", {
                use pyo3::prelude::*;
...
impl LiteralValue {
    pub fn get_type(&self) -> DataType {
        match self {
            ...
            LiteralValue::Time(_, tu) => DataType::Time(*tu),
            LiteralValue::Timestamp(_, tu, tz) => DataType::Timestamp(*tu, tz.clone()),
            LiteralValue::Float64(_) => DataType::Float64,
            LiteralValue::Decimal(_, precision, scale) => {
                DataType::Decimal128(*precision as usize, *scale as usize)
            }
            LiteralValue::Series(series) => series.data_type().clone(),
            #[cfg(feature = "python")]
            LiteralValue::Python(_) => DataType::Python,
        }
    }

    pub fn to_series(&self) -> Series {
        let result = match self {
            LiteralValue::Null => NullArray::full_null("literal", &DataType::Null, 1).into_series(),
            LiteralValue::Boolean(val) => {
                BooleanArray::from(("literal", [*val].as_slice())).into_series()
            }
            LiteralValue::Utf8(val) => {
                Utf8Array::from(("literal", [val.as_str()].as_slice())).into_series()
            }
            LiteralValue::Binary(val) => {
                BinaryArray::from(("literal", val.as_slice())).into_series()
            }
            LiteralValue::Int32(val) => {
                Int32Array::from(("literal", [*val].as_slice())).into_series()
            }
            LiteralValue::UInt32(val) => {
                UInt32Array::from(("literal", [*val].as_slice())).into_series()
            }
            LiteralValue::Int64(val) => {
                Int64Array::from(("literal", [*val].as_slice())).into_series()
            }
            LiteralValue::UInt64(val) => {
                UInt64Array::from(("literal", [*val].as_slice())).into_series()
            }
            LiteralValue::Date(val) => {
                let physical = Int32Array::from(("literal", [*val].as_slice()));
                DateArray::new(Field::new("literal", self.get_type()), physical).into_series()
            }
            LiteralValue::Time(val, ..) => {
                let physical = Int64Array::from(("literal", [*val].as_slice()));
                TimeArray::new(Field::new("literal", self.get_type()), physical).into_series()
            }
            LiteralValue::Timestamp(val, ..) => {
                let physical = Int64Array::from(("literal", [*val].as_slice()));
                TimestampArray::new(Field::new("literal", self.get_type()), physical).into_series()
            }
            LiteralValue::Float64(val) => {
                Float64Array::from(("literal", [*val].as_slice())).into_series()
            }
            LiteralValue::Decimal(val, ..) => {
            ...
            LiteralValue::Series(series) => series.clone().rename("literal"),
            #[cfg(feature = "python")]
            LiteralValue::Python(val) => {
                PythonArray::from(("literal", vec![val.0.clone()])).into_series()
            }
        };
        result
    }
    ...
    pub fn display_sql<W: Write>(&self, buffer: &mut W) -> io::Result<()> {
        ...
        match self {
            LiteralValue::Null => write!(buffer, "NULL"),
            LiteralValue::Boolean(v) => write!(buffer, "{}", v),
            LiteralValue::Int32(val) => write!(buffer, "{}", val),
            LiteralValue::UInt32(val) => write!(buffer, "{}", val),
            LiteralValue::Int64(val) => write!(buffer, "{}", val),
            LiteralValue::UInt64(val) => write!(buffer, "{}", val),
            LiteralValue::Float64(val) => write!(buffer, "{}", val),
            LiteralValue::Utf8(val) => write!(buffer, "'{}'", val),
            LiteralValue::Date(val) => write!(buffer, "DATE '{}'", display_date32(*val)),
            ...
            LiteralValue::Timestamp(val, tu, tz) => write!(
                buffer,
                "TIMESTAMP '{}'",
            ...
            | LiteralValue::Series(..)
            | LiteralValue::Time(..)
            | LiteralValue::Binary(..) => display_sql_err,
            #[cfg(feature = "python")]
            LiteralValue::Python(..) => display_sql_err,
        }
    }
