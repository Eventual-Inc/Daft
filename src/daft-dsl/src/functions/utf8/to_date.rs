...
impl FunctionEvaluator for ToDateEvaluator {
                ...
                (Ok(data_field), Ok(format_field)) => {
                    match (&data_field.dtype, &format_field.dtype) {
                        (DataType::Utf8, DataType::Utf8) => {
                            Ok(Field::new(data_field.name, DataType::Date))
                        }
                        _ => Err(DaftError::TypeError(format!(
                            "Expects inputs to todate to be utf8 and utf8, but received {data_field} and {format_field}",
                        ))),
                    }
                }
    ...
    fn evaluate(&self, inputs: &[Series], _: &FunctionExpr) -> DaftResult<Series> {
        match inputs {
            [data, format] => data.utf8_to_date(format),
