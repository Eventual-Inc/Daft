...
pub(super) struct ReplaceAllEvaluator {}

impl FunctionEvaluator for ReplaceAllEvaluator {
                ...
                (Ok(data_field), Ok(pattern_field), Ok(replacement_field)) => {
                    match (&data_field.dtype, &pattern_field.dtype, &replacement_field.dtype) {
                        ...
                        _ => Err(DaftError::TypeError(format!(
                            "Expects inputs to Replace_All to be utf8, but received {data_field} and {pattern_field} and {replacement_field}",
                        ))),
                    }
                }
        ...
        match inputs {
            [data, pattern, replacement] => {
                let regex = match expr {
                    ...
                    _ => panic!("Expected Utf8 Replace_All Expr, got {expr}"),
                };
                data.utf8_replace(pattern, replacement, *regex, true)
            }
