...
#[cfg(feature = "python")]
type MemoryStore = HashMap<String, std::sync::Arc<pyo3::PyObject>>;
#[cfg(feature = "python")]
    ...
    fn call(&self, args: FunctionArgs<Series>) -> DaftResult<Series> {
        ...
        Python::with_gil(|py| {
            for i in 0..len {
                ...
                let obj_opt: Option<std::sync::Arc<pyo3::PyObject>> = match &v_dtype {
                    ...
                    DataType::Utf8 => match value.utf8().ok().and_then(|a| a.get(i)) {
                        Some(s) => {
                            ...
                            Some(std::sync::Arc::new(
                                pystr.into_pyobject(py).unwrap().unbind().into(),
                            ))
                        }
                        None => None,
                    },
                                ...
                                .and_then(|larr| larr.iter().nth(i).flatten())
                            {
                                Some(sub) => match sub.f64().ok() {
                                    Some(farr) => {
                                        ...
                                        Some(std::sync::Arc::new(
                                            pylist.into_pyobject(py).unwrap().unbind().into(),
                                        ))
                                    }
                                    None => None,
                                },
    ...
    fn call(&self, args: FunctionArgs<Series>) -> DaftResult<Series> {
        ...
        Python::with_gil(|py| {
            for i in 0..k_arr.len() {
                ...
                let dict_obj: pyo3::PyObject = pydict.into_pyobject(py).unwrap().unbind().into();
