...
pub struct ListExcept;

#[typetag::serde]
impl ScalarUDF for ListExcept {
    ...
    fn call(
        ...
        // Allow broadcasting either side when the other is length 1.
        let (lhs_b, rhs_b);
        let (lhs_ref, rhs_ref) = match (lhs.len(), rhs.len()) {
            ...
            _ => {
                return Err(common_error::DaftError::ValueError(format!(
                    "list_except: input lengths must match or broadcast, got {} vs {}",
                    lhs.len(),
                    rhs.len()
                )));
            }
