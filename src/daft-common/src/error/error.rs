...
macro_rules! ensure {
    ($cond:expr, $msg:expr) => {
        if !$cond {
            return Err($crate::error::DaftError::ComputeError($msg.to_string()));
        }
    };
    ($cond:expr, $variant:ident: $($msg:tt)*) => {
        if !$cond {
            return Err($crate::error::DaftError::$variant(format!($($msg)*)));
        }
    };
}
...
macro_rules! value_err {
    ($($arg:tt)*) => {
        return Err($crate::error::DaftError::ValueError(format!($($arg)*)))
    };
}
