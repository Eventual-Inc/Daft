...
/// Per-shuffle metadata registered by the worker `ShuffleWriteSink` before any
/// raw entries arrive. Lives in `ShuffleFlightServer::shuffle_specs`. The server
/// uses it to decode + partition raw IPC files during `run_repartition`.
///
/// `repartition_threshold_bytes` is carried per-shuffle (rather than a server-wide
/// constant) because the worker is the one that reads `DaftExecutionConfig`; the
/// server itself has no direct config access.
#[derive(Clone)]
struct ShuffleSpec {
    ...
    repartition_threshold_bytes: usize,
}

/// One raw IPC file registered by a single small-mode map task. The file holds
/// unpartitioned data; the server will decode it, run the partition kernel,
/// and produce per-partition entries keyed by `promise_ref_ids[partition_idx]`.
struct RawEntry {
    file_path: String,
    size_bytes: usize,
    /// Pre-allocated ref_ids that downstream readers will use to fetch each
    /// output partition. One per `partition_idx`, generated by the worker via
    /// `partition_ref_id(input_id, partition_idx)`.
    promise_ref_ids: Vec<u64>,
}

/// Accumulator of raw IPC files waiting to be repartitioned. Once `total_bytes`
/// crosses the server's threshold, a background pass takes the accumulated
/// entries and runs `run_repartition`. `repartition_in_flight` serializes
/// passes so only one repartition job runs at a time per shuffle.
///
/// `pass_complete` fires whenever `repartition_in_flight` flips back to false
/// (after either a successful publish or a failure restore) — the first-read
/// flush path waits on it instead of polling.
struct RawPending {
    entries: Vec<RawEntry>,
    total_bytes: usize,
    repartition_in_flight: bool,
    pass_complete: Arc<tokio::sync::Notify>,
}

impl Default for RawPending {
    fn default() -> Self {
        Self {
            entries: Vec::new(),
            total_bytes: 0,
            repartition_in_flight: false,
            pass_complete: Arc::new(tokio::sync::Notify::new()),
        }
    }
}
...
pub struct ShuffleFlightServer {
    ...
    // ---- Server-side repartition state (Phase: server_side_repartition) ----
    /// Per-shuffle metadata, populated by `register_shuffle_spec` before any
    /// raw entries arrive. Absence means "legacy path" — `register_shuffle_partitions`
    /// will fall through to the old `shuffle_partitions` map.
    shuffle_specs: Arc<Mutex<HashMap<u64, ShuffleSpec>>>,
    /// Accumulator of raw IPC files per shuffle.
    raw_pending: Arc<Mutex<HashMap<u64, RawPending>>>,
    /// Per-shuffle first-read flush gate. The inner `Mutex<bool>` serializes
    /// concurrent first-readers; `true` means a flush has already completed
    /// successfully and subsequent reads can skip. On flush failure the bool
    /// stays `false` so retries can run again — unlike `OnceCell`, transient
    /// errors are not memoized.
    first_read_flushed: Arc<Mutex<HashMap<u64, Arc<Mutex<bool>>>>>,
    /// Monotonic counter for naming server-side repartition output files.
    repartition_seq: Arc<AtomicU64>,
}
...
impl ShuffleFlightServer {
    ...
    pub fn with_seal_config(seal_cfg: SealConfig) -> Self {
        Self {
            ...
            shuffle_specs: Arc::new(Mutex::new(HashMap::new())),
            raw_pending: Arc::new(Mutex::new(HashMap::new())),
            first_read_flushed: Arc::new(Mutex::new(HashMap::new())),
            repartition_seq: Arc::new(AtomicU64::new(1)),
        }
    }
    ...
    /// Register the partition spec for a shuffle. Called once by `ShuffleWriteSink`
    /// before any raw entries arrive. Idempotent: the first registration wins.
    /// Subsequent calls are no-ops (asserting in debug builds that the spec
    /// matches) — concurrent workers may all call this with identical args.
    #[allow(clippy::too_many_arguments)]
    pub async fn register_shuffle_spec(
        &self,
        shuffle_id: u64,
        repartition_spec: RepartitionSpec,
        num_partitions: usize,
        ...
        let mut specs = self.shuffle_specs.lock().await;
        specs.entry(shuffle_id).or_insert(ShuffleSpec {
            repartition_spec,
            num_partitions,
            schema,
            compression,
            shuffle_dirs,
            repartition_threshold_bytes,
        });
        Ok(())
    }

    /// Register a raw IPC file written by a small-mode map task. The file holds
    /// unpartitioned data; `promise_ref_ids` are the `partition_ref_id(input_id, i)`
    /// values for each output partition `i` — readers will eventually ask for
    /// these specific ref_ids, and the server resolves them by running
    /// `run_repartition` over accumulated raw files.
    pub async fn register_raw_entry(
        &self,
        shuffle_id: u64,
        file_path: String,
        size_bytes: usize,
        ...
        // Look up the per-shuffle threshold. `register_shuffle_spec` is always
        // called by the sink before its first `register_raw_entry`, so the spec
        // is present here in practice. Missing spec → background passes are
        // disabled (defaults to flush-on-first-read).
        let threshold_bytes = {
            let specs = self.shuffle_specs.lock().await;
            specs
                .get(&shuffle_id)
                .map(|s| s.repartition_threshold_bytes)
                .unwrap_or(0)
        };
        let trigger_repartition: Option<Vec<RawEntry>> = {
            let mut pending_map = self.raw_pending.lock().await;
            let pending = pending_map.entry(shuffle_id).or_default();
            pending.entries.push(RawEntry {
                file_path,
                size_bytes,
                promise_ref_ids,
            });
            pending.total_bytes += size_bytes;
            if threshold_bytes > 0
                && pending.total_bytes >= threshold_bytes
                && !pending.repartition_in_flight
            {
                pending.repartition_in_flight = true;
        ...
        if let Some(entries) = trigger_repartition {
            let server = self.clone();
            tokio::spawn(async move {
                if let Err(e) = server.run_repartition(shuffle_id, entries).await {
                    tracing::warn!(
                        target: "daft_shuffles::server_repartition",
                        shuffle_id = shuffle_id,
                        error = %e,
                        "background repartition pass failed; entries restored for first-read flush"
                    );
                }
            });
        }
    ...
    /// Decode the raw IPC files in `entries`, apply the partition kernel, write
    /// one mega-file with per-partition byte ranges via `write_partitions_one_shot`,
    /// then atomically publish per-partition `PartitionCache` entries keyed by
    /// `promise_ref_ids[partition_idx]`. On any error before publish, raw entries
    /// are restored to `raw_pending` so the first-read flush (or a later trigger)
    /// can retry.
    async fn run_repartition(&self, shuffle_id: u64, entries: Vec<RawEntry>) -> DaftResult<()> {
        if entries.is_empty() {
            self.clear_repartition_flag(shuffle_id).await;
            return Ok(());
        }
        let spec = {
            let specs = self.shuffle_specs.lock().await;
            specs.get(&shuffle_id).cloned()
        };
        let Some(spec) = spec else {
            // No spec → caller shouldn't have called us; restore and bail.
            self.restore_raw_entries(shuffle_id, entries).await;
            return Err(DaftError::InternalError(format!(
                "run_repartition called for shuffle {} without a registered spec",
                shuffle_id
            )));
        };

        let result = self.run_repartition_inner(shuffle_id, &entries, &spec).await;

        match result {
            Ok(()) => {
                // Source files have already been unlinked by run_repartition_inner
                // after the atomic publish.
                self.clear_repartition_flag(shuffle_id).await;
                Ok(())
            }
            Err(e) => {
                // Decode/partition/write failed before publish. Put the raw
                // entries back so first-read flush can retry.
                self.restore_raw_entries(shuffle_id, entries).await;
                Err(e)
            }
        }
    ...
    async fn clear_repartition_flag(&self, shuffle_id: u64) {
        let notify = {
            let mut pending_map = self.raw_pending.lock().await;
            pending_map.get_mut(&shuffle_id).map(|pending| {
                pending.repartition_in_flight = false;
                pending.pass_complete.clone()
            })
        };
    ...
    async fn restore_raw_entries(&self, shuffle_id: u64, entries: Vec<RawEntry>) {
        let notify = {
            let mut pending_map = self.raw_pending.lock().await;
            let pending = pending_map.entry(shuffle_id).or_default();
            for e in entries {
                pending.total_bytes += e.size_bytes;
    ...
    async fn run_repartition_inner(
        &self,
        shuffle_id: u64,
        entries: &[RawEntry],
        spec: &ShuffleSpec,
        ...
        let num_partitions = spec.num_partitions;
        let source_paths: Vec<String> = entries.iter().map(|e| e.file_path.clone()).collect();

        // 1. Decode all raw files → MicroPartition (concat-on-the-fly).
        let combined = {
            let schema = schema.clone();
            let source_paths = source_paths.clone();
            get_io_runtime(true)
                .spawn_blocking(move || -> DaftResult<MicroPartition> {
                    decode_raw_files_to_micropartition(&source_paths, &schema)
                })
                .await??
        };

        if combined.is_empty() {
            // Nothing to write. Still publish empty per-partition caches so
            // readers find an entry instead of a "not found" error.
            self.publish_empty_partitions(shuffle_id, entries, &schema).await?;
            unlink_files(&source_paths);
            return Ok(());
        }

        // 2. Apply the partition kernel.
        let partitioned: Vec<MicroPartition> = match &spec.repartition_spec {
            RepartitionSpec::Hash(cfg) => {
                let bound =
                    BoundExpr::bind_all(&cfg.by, &schema).map_err(|e| DaftError::InternalError(
                        format!("BoundExpr::bind_all failed for shuffle {}: {}", shuffle_id, e),
                    ))?;
                combined.partition_by_hash(&bound, num_partitions)?
            }
            RepartitionSpec::Random(cfg) => {
                combined.partition_by_random(num_partitions, cfg.seed.unwrap_or(0))?
            }
            RepartitionSpec::Range(_) => {
                return Err(DaftError::InternalError(format!(
                    "Range partitioning is not yet supported by server-side repartition (shuffle {})",
                    shuffle_id
                )));
            }
        };

        // 3. Write one mega-file with per-partition byte ranges.
        let partitions_per_output: Vec<Vec<MicroPartition>> =
            partitioned.into_iter().map(|p| vec![p]).collect();
        let mega_input_id = self.repartition_seq.fetch_add(1, Ordering::Relaxed) as u32;
        let caches = write_partitions_one_shot(
            ...
            schema.clone(),
            spec.compression,
            partitions_per_output,
        )
        .await?;
        assert_eq!(caches.len(), num_partitions);

        // 4. Atomic publish: register one PartitionCache per (entry, partition_idx)
        // by cloning the mega-file's per-partition cache and rewriting its
        // partition_ref_id to match each promise. Multiple entries that promise
        // the same partition_idx all resolve to the same mega-file slice; the
        // server's read path (get_shuffle_file_specs) dedupes by file path and
        // serves the slice once per request.
        {
            let mut partitions = self.shuffle_partitions.lock().await;
            for entry in entries {
                for (partition_idx, &promise_ref_id) in entry.promise_ref_ids.iter().enumerate() {
                    if partition_idx >= caches.len() {
                        break;
                    }
                    let mut cache = caches[partition_idx].clone();
                    cache.partition_ref_id = promise_ref_id;
                    partitions.insert(
                        FlightPartitionKey {
                            shuffle_id,
                            partition_ref_id: promise_ref_id,
                        },
                        cache,
                    );
                }
        ...
        // 5. Unlink source raw files only after publish succeeds.
        unlink_files(&source_paths);
        Ok(())
    }

    async fn publish_empty_partitions(
        &self,
        shuffle_id: u64,
        entries: &[RawEntry],
        schema: &SchemaRef,
        ...
        let mut partitions = self.shuffle_partitions.lock().await;
        for entry in entries {
            for &promise_ref_id in &entry.promise_ref_ids {
                partitions.insert(
                    FlightPartitionKey {
                        shuffle_id,
                        partition_ref_id: promise_ref_id,
    ...
    /// Flush any residual raw entries for `shuffle_id` through one synchronous
    /// repartition pass. Called by the first reader for the shuffle (see
    /// `ensure_first_read_flushed`). Waits for any in-flight background pass
    /// to clear before draining — uses a `Notify` on `RawPending` so the wake
    /// is event-driven, not polled.
    async fn flush_residual(&self, shuffle_id: u64) -> DaftResult<()> {
        let entries = loop {
            // Critical ordering: register interest in the `Notified` future
            // BEFORE re-checking `repartition_in_flight` under the lock.
            // `Notify::notify_waiters` only wakes existing waiters; if we
            // checked the flag first and the pass completed before we set up
            // the waiter, we'd miss the wakeup and hang.
            let notified = {
                let mut pending_map = self.raw_pending.lock().await;
                let pending = match pending_map.get_mut(&shuffle_id) {
                    Some(p) => p,
                    None => return Ok(()),
                };
                if pending.repartition_in_flight {
                    // Background pass mid-flight; wait for it to complete and
                    // re-check.
                    pending.pass_complete.clone().notified_owned()
        ...
        self.run_repartition(shuffle_id, entries).await
    }

    /// First-read gate. Concurrent callers serialize on a per-shuffle async
    /// Mutex; the first to acquire it runs `flush_residual` if not already
    /// flushed. On success the per-shuffle bool flips to `true` and subsequent
    /// readers fast-path. On failure the bool stays `false`, so a client retry
    /// will re-attempt the flush rather than seeing a memoized error.
    async fn ensure_first_read_flushed(&self, shuffle_id: u64) -> DaftResult<()> {
        let has_spec = {
            let specs = self.shuffle_specs.lock().await;
            specs.contains_key(&shuffle_id)
        };
        if !has_spec {
            // Legacy shuffles never registered a spec — nothing to flush.
            return Ok(());
        }
        let gate = {
            let mut map = self.first_read_flushed.lock().await;
            map.entry(shuffle_id)
        ...
        self.flush_residual(shuffle_id).await?;
        *done = true;
        Ok(())
    }
    ...
    async fn get_shuffle_file_specs(
        ...
        //
        // Dedupe by (path, start, end): the server-side repartition path publishes one
        // mega-file slice under N aliased promise ref_ids (one per map task that
        // contributed). Without dedup, a ticket carrying all N promise refs would push
        // the same byte range N times and the server would return N duplicate copies
        // of the rows. Whole-file specs likewise dedupe by path. Aliased entries are
        // expected on the new server-side repartition path; on the legacy path each
        // (path, start, end) is unique so the dedupe is a no-op.
        let mut whole_specs = Vec::new();
        let mut whole_seen: HashSet<String> = HashSet::new();
        let mut ranges_by_path: HashMap<String, Vec<(u64, u64)>> = HashMap::new();
        let mut range_seen: HashSet<(String, u64, u64)> = HashSet::new();
        ...
        for partition_ref_id in partition_ref_ids {
            ...
            match &cache.byte_ranges {
                ...
                None => {
                    ...
                    {
                        if !whole_seen.insert(path.clone()) {
                            continue;
                        }
    ...
    pub async fn get_partition_local(
        ...
        // First-read implicit flush: if a `ShuffleSpec` is registered for this
        // shuffle, drain any residual raw entries through one synchronous
        // repartition pass before resolving the refs. No-op for legacy shuffles.
        self.ensure_first_read_flushed(shuffle_id).await?;
...
/// Decode each raw IPC file synchronously and concat all batches into a single
/// `MicroPartition` over `schema`. Used by the server-side repartition path to
/// turn N raw map-task files into one input for the partition kernel.
fn decode_raw_files_to_micropartition(
    paths: &[String],
    schema: &SchemaRef,
    ...
    for path in paths {
        ...
        arrow_batches_to_daft_into(&arrow_batches, &fields, schema, &mut all_batches)?;
    }
    if all_batches.is_empty() {
        return Ok(MicroPartition::empty(Some(schema.clone())));
    }
    ...
    for path in paths {
        if let Err(e) = std::fs::remove_file(path) {
            tracing::warn!(
                ...
                path = %path,
                error = %e,
                "failed to unlink raw source after publish"
            );
        }
    }
}
...
impl FlightService for ShuffleFlightServer {
    ...
    async fn do_get(
        ...
        // First-read implicit flush: residual raw entries get repartitioned
        // before we attempt to resolve refs. No-op for legacy (non-spec) shuffles.
        self.ensure_first_read_flushed(shuffle_id)
            .await
            .map_err(|e| Status::internal(format!("first-read flush failed: {}", e)))?;
