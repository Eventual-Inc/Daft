...
fn get_shuffle_dirs(
    ...
    // <base_dir>/daft_shuffle/<query_id>/<shuffle_id>/<cache_id>
    base_dirs
        .iter()
        .map(|dir| {
            format!(
                "{}/daft_shuffle/{}/{}/{}",
                dir, query_id, shuffle_id, cache_id
            )
        })
        .collect()
}
...
impl InProgressShuffleCache {
    pub fn try_new(
        ...
        let shuffle_dirs = get_shuffle_dirs(dirs, shuffle_id, query_id, cache_id.clone());
