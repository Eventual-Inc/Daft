...
impl SQLPlanner<'_> {
    ...
    pub(crate) fn plan_query(&mut self, query: &Query) -> SQLPlannerResult<LogicalPlanBuilder> {
        ...
        // and ensure that LIMIT is a child node of OFFSET.
        if let Some(offset) = &query.offset {
            let offset_expr = self.plan_expr(&offset.value)?;
            match offset_expr.as_ref() {
                Expr::Literal(LiteralValue::Int64(offset)) if *offset >= 0 => {
                    self.update_plan(|plan| plan.offset(*offset as u64))?;
                }
                Expr::Literal(LiteralValue::Int64(offset)) => invalid_operation_err!(
...
/// /// This function examines various clauses and options in the provided [Query]
