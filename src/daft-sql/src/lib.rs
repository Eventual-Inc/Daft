...
mod tests {
    ...
    use daft_catalog::{Table, TableSource, TempTable};
    ...
    fn planner() -> SQLPlanner<'static> {
        ...
        _ = session.create_temp_table("tbl1", TempTable::source(tbl_1()));
        _ = session.create_temp_table("tbl2", TempTable::source(tbl_2()));
        _ = session.create_temp_table("tbl3", TempTable::source(tbl_3()));
