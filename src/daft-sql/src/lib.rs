...
mod tests {
    ...
    use daft_catalog::{Table, TableSource, View};
    ...
    fn planner() -> SQLPlanner<'static> {
        ...
        _ = session.create_temp_table("tbl1", &tbl_1().into());
        _ = session.create_temp_table("tbl2", &tbl_2().into());
        _ = session.create_temp_table("tbl3", &tbl_3().into());
