...
use error::{PlannerError, SQLPlannerResult};
use sqlparser::{
    ast::{
...
impl SQLContext {
    ...
    pub fn get_table(&self, name: &str) -> SQLPlannerResult<LogicalPlanRef> {
        self.tables
            .get(name)
            .cloned()
            .ok_or(PlannerError::table_not_found(name.to_string()))
    }
