mod ascii;
mod capitalize;
mod case;
mod chr;
mod contains;
mod count_matches;
mod damerau_levenshtein;
mod endswith;
mod find;
mod find_in_set;
mod hamming;
mod ilike;
mod jaro;
mod jaro_winkler;
mod left;
mod length_bytes;
mod levenshtein;
mod like;
mod lower;
mod lpad;
mod lstrip;
mod normalize;
mod overlay;
pub(crate) mod pad;
mod regexp_count;
mod regexp_extract;
mod regexp_extract_all;
mod regexp_match;
mod repeat;
mod replace;
mod reverse;
mod right;
mod rpad;
mod rstrip;
mod soundex;
mod space;
mod split;
mod startswith;
mod strip;
mod substr;
mod substring_index;
mod to_date;
mod to_datetime;
mod translate;
mod upper;
mod url;
pub(crate) mod utils;

pub use ascii::*;
pub use capitalize::*;
pub use case::*;
pub use chr::*;
pub use contains::*;
pub use count_matches::*;
pub use damerau_levenshtein::*;
pub use endswith::*;
pub use find::*;
pub use find_in_set::*;
pub use hamming::*;
pub use ilike::*;
pub use jaro::*;
pub use jaro_winkler::*;
pub use left::*;
pub use length_bytes::*;
pub use levenshtein::*;
pub use like::*;
pub use lower::*;
pub use lpad::*;
pub use lstrip::*;
pub use normalize::*;
pub use overlay::*;
pub use regexp_count::*;
pub use regexp_extract::*;
pub use regexp_extract_all::*;
pub use regexp_match::*;
pub use repeat::*;
pub use replace::*;
pub use reverse::*;
pub use right::*;
pub use rpad::*;
pub use rstrip::*;
pub use soundex::*;
pub use space::*;
pub use split::*;
pub use startswith::*;
pub use strip::*;
pub use substr::*;
pub use substring_index::*;
pub use to_date::*;
pub use to_datetime::*;
pub use translate::*;
pub use upper::*;
pub use url::*;

pub struct Utf8Functions;

impl daft_dsl::functions::FunctionModule for Utf8Functions {
    fn register(parent: &mut daft_dsl::functions::FunctionRegistry) {
        parent.add_fn(CamelCase);
        parent.add_fn(Capitalize);
        parent.add_fn(Contains);
        parent.add_fn(CountMatches);
        parent.add_fn(DamerauLevenshteinDistance);
        parent.add_fn(EndsWith);
        parent.add_fn(Find);
        parent.add_fn(FindInSet);
        parent.add_fn(HammingDistance);
        parent.add_fn(ILike);
        parent.add_fn(JaroSimilarity);
        parent.add_fn(JaroWinklerSimilarity);
        parent.add_fn(KebabCase);
        parent.add_fn(Left);
        parent.add_fn(LengthBytes);
        parent.add_fn(LevenshteinDistance);
        parent.add_fn(Like);
        parent.add_fn(Lower);
        parent.add_fn(LPad);
        parent.add_fn(LStrip);
        parent.add_fn(Normalize);
        parent.add_fn(Overlay);
        parent.add_fn(RegexpCount);
        parent.add_fn(RegexpExtract);
        parent.add_fn(RegexpExtractAll);
        parent.add_fn(RegexpMatch);
        parent.add_fn(RegexpReplace);
        parent.add_fn(RegexpSplit);
        parent.add_fn(Repeat);
        parent.add_fn(Replace);
        parent.add_fn(Reverse);
        parent.add_fn(Right);
        parent.add_fn(RPad);
        parent.add_fn(RStrip);
        parent.add_fn(Soundex);
        parent.add_fn(Space);
        parent.add_fn(Strip);
        parent.add_fn(SnakeCase);
        parent.add_fn(Split);
        parent.add_fn(StartsWith);
        parent.add_fn(Substr);
        parent.add_fn(SubstringIndex);
        parent.add_fn(TitleCase);
        parent.add_fn(ToDate);
        parent.add_fn(ToDatetime);
        parent.add_fn(Translate);
        parent.add_fn(Upper);
        parent.add_fn(UrlEncode);
        parent.add_fn(UrlDecode);
        parent.add_fn(UpperCamelCase);
        parent.add_fn(UpperKebabCase);
        parent.add_fn(UpperSnakeCase);
        parent.add_fn(Ascii);
        parent.add_fn(Chr);
    }
}
