...
static ARROW_DDSKETCH_ITEM_FIELDS: LazyLock<Vec<daft_arrow::datatypes::Field>> =
    LazyLock::new(|| {
...
/// The corresponding arrow DataType of Vec<DDSketch> when serialized as an arrow2 array
pub static ARROW_DDSKETCH_DTYPE: LazyLock<daft_arrow::datatypes::DataType> = LazyLock::new(|| {
...
#[must_use]
pub fn into_arrow(sketches: Vec<Option<DDSketch>>) -> Box<dyn daft_arrow::array::Array> {
    if sketches.is_empty() {
        return todo!();
        // return daft_arrow::array::StructArray::new_empty(ARROW_DDSKETCH_DTYPE.clone()).to_boxed();
    }
