...
pub struct TaskInfo {
    pub id: u32,
    /// The distributed plan node that dispatched this task. Equal to
    /// `TaskContext::last_node_id` and to `node_ids.last()`. Conceptually the
    /// same kind of value as `LocalNodeContext::origin_node_id` (a distributed
    /// pipeline node ID); kept here as a named, primary field because it is
    /// the natural grouping key for "tasks submitted by node X".
