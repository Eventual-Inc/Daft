...
impl TreeDisplay for crate::physical_plan::PhysicalPlan {
    fn display_as(&self, level: DisplayLevel) -> String {
        match self {
            ...
            Self::Split(split) => split.display_as(level),
            Self::Sample(sample) => sample.display_as(level),
            Self::MonotonicallyIncreasingId(id) => id.display_as(level),
            Self::Coalesce(coalesce) => coalesce.display_as(level),
            Self::Flatten(flatten) => flatten.display_as(level),
            Self::FanoutRandom(fanout) => fanout.display_as(level),
            Self::FanoutByHash(fanout) => fanout.display_as(level),
            Self::FanoutByRange(fanout) => fanout.display_as(level),
            Self::ReduceMerge(reduce) => reduce.display_as(level),
