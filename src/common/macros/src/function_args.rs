        ...
        |expr: #daft_dsl::ExprRef, name: &str| {
            if let #daft_dsl::Expr::Literal(val) = expr.as_ref() {
                ...
                std::result::Result::Err(
                    daft_common_error::DaftError::ValueError(format!("Expected argument `{}` to be a literal, received: {}", name, expr))
                )
            }
        }
    };
...
fn derive_for_type(
            ...
            match (c, t) {
                (ArgCardinality::Required, ArgType::Generic) => quote! {
                    unnamed
                        ...
                        .ok_or_else(|| daft_common_error::DaftError::ValueError(format!("Required argument `{}` not found", #n)))?
                },
                ...
                (ArgCardinality::Variadic, ArgType::Concrete(_)) => quote! {
                    unnamed
                        .drain(..)
                        .map(|val| #daft_core::lit::FromLiteral::try_from_literal(&to_lit(val, #n)?))
                        .collect::<daft_common_error::DaftResult<_>>()?
                },
            }
        });
    ...
    quote! {
        impl std::convert::TryFrom<#daft_dsl::functions::FunctionArgs<#ty>> for #impl_type {
            type Error = daft_common_error::DaftError;

            fn try_from(args: #daft_dsl::functions::FunctionArgs<#ty>) -> daft_common_error::DaftResult<Self> {
                ...
                if !unnamed.is_empty() {
                    return std::result::Result::Err(
                        daft_common_error::DaftError::ValueError(format!("Expected {} arguments, received: {}", #num_fields, #num_fields + unnamed.len()))
                    );
                }

                if !named.is_empty() {
                    return std::result::Result::Err(
                        daft_common_error::DaftError::ValueError(format!("Expected argument names [{}], received: {}",
                            #named_args,
                            named.keys()
                                .map(|s| format!("`{}`", s))
                                .collect::<Vec<_>>()
