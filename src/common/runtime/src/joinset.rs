...
pub enum OrderableJoinSet<T> {
    Ordered(OrderedJoinSet<T>),
    Unordered(JoinSet<T>),
}

impl<T: Send + 'static> OrderableJoinSet<T> {
    pub fn new(ordered: bool) -> Self {
        if ordered {
            Self::Ordered(OrderedJoinSet::new())
        } else {
            Self::Unordered(create_join_set())
        }
    }
    ...
    pub fn spawn_on(
        ...
        match self {
            Self::Ordered(join_set) => join_set.spawn_on(task, handle),
            Self::Unordered(join_set) => join_set.spawn_on(task, handle),
        }
    }

    pub fn spawn_front(&mut self, task: impl Future<Output = T> + Send + 'static) -> JoinSetId {
        match self {
            Self::Ordered(join_set) => join_set.spawn_front(task),
            Self::Unordered(join_set) => join_set.spawn(task),
        }
    }

    pub fn spawn_front_on(
        &mut self,
        task: impl Future<Output = T> + Send + 'static,
        handle: &tokio::runtime::Handle,
    ) -> JoinSetId {
        match self {
            Self::Ordered(join_set) => join_set.spawn_front_on(task, handle),
            Self::Unordered(join_set) => join_set.spawn(task),
        }
    }
    ...
    pub fn is_empty(&self) -> bool {
        match self {
            Self::Ordered(join_set) => join_set.num_pending() == 0,
...
impl<T: Send + 'static> OrderedJoinSet<T> {
    ...
    pub fn spawn_on(
        ...
        let id = self.join_set.spawn_on(task, handle);
        self.order.push_back(id.clone());
        id
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_ordered_joinset_spawn_front() {
        let mut join_set = OrderedJoinSet::new();

        // Spawn some tasks normally
        join_set.spawn(async { 2 });
        join_set.spawn(async { 3 });

        // Spawn a task at the front
        join_set.spawn_front(async { 1 });

        // The front task should come first
        assert_eq!(join_set.join_next().await.unwrap().unwrap(), 1);
        assert_eq!(join_set.join_next().await.unwrap().unwrap(), 2);
        assert_eq!(join_set.join_next().await.unwrap().unwrap(), 3);
        assert!(join_set.join_next().await.is_none());
    ...
    #[tokio::test]
    async fn test_ordered_joinset_spawn_front_mixed() {
        let mut join_set = OrderedJoinSet::new();

        // Mix of spawn and spawn_front
        join_set.spawn(async { 3 });
        join_set.spawn_front(async { 1 });
        join_set.spawn(async { 4 });
        join_set.spawn_front(async { 2 });

        // Order should be: front items in reverse order, then normal items in order
        // spawn_front(1), spawn_front(2) -> [2, 1] at front
        // spawn(3), spawn(4) -> [3, 4] at back
        // Final order: [2, 1, 3, 4]
        assert_eq!(join_set.join_next().await.unwrap().unwrap(), 2);
        assert_eq!(join_set.join_next().await.unwrap().unwrap(), 1);
        assert_eq!(join_set.join_next().await.unwrap().unwrap(), 3);
        assert_eq!(join_set.join_next().await.unwrap().unwrap(), 4);
        assert!(join_set.join_next().await.is_none());
    }
}
