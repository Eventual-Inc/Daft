...
use super::{
    object_source::{GetResult, ObjectSource},
    Result,
};
use crate::{
    object_source::{self, FileMetadata, LSResult},
    stats::IOStatsRef,
};
    ...
    async fn iter_dir(
        ...
        if meta.file_type().is_file() {
            ...
            return Ok(futures::stream::iter([Ok(FileMetadata {
                ...
                size: Some(meta.len()),
                filetype: object_source::FileType::File,
            })])
            .boxed());
        }
...
mod tests {
    ...
    use crate::{
        object_source::{FileMetadata, FileType, ObjectSource}, local::LocalSource, Result,
    };

    async fn write_bytes_to_local_file(
    ...
    async fn test_local_full_get() -> Result<()> {
        ...
        let bytes = write_bytes_to_local_file(&mut file1).await?;
    ...
    async fn test_local_full_ls() -> Result<()> {
        ...
        write_bytes_to_local_file(&mut file1).await?;
        let mut file2 = tempfile::NamedTempFile::new_in(dir.path()).unwrap();
        write_bytes_to_local_file(&mut file2).await?;
        let mut file3 = tempfile::NamedTempFile::new_in(dir.path()).unwrap();
        write_bytes_to_local_file(&mut file3).await?;
