...
pub struct DaftExecutionConfig {
    ...
    pub shuffle_map_spill_threshold: Option<usize>,
}
...
impl DaftExecutionConfig {
    ...
    const ENV_DAFT_SHUFFLE_MAP_SPILL_THRESHOLD: &'static str = "DAFT_SHUFFLE_MAP_SPILL_THRESHOLD";
    ...
    pub fn from_env() -> Self {
        ...
        if let Some(val) = parse_number_from_env(
            Self::ENV_DAFT_SHUFFLE_MAP_SPILL_THRESHOLD,
            cfg.shuffle_map_spill_threshold.unwrap_or(0),
        ) {
            cfg.shuffle_map_spill_threshold = Some(val);
        }
