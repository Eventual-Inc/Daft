...
use crate::{dtype::DataType, field::Field, prelude::TimeUnit, schema::Schema};

/// Treat any input issue as a ValueError.
#[macro_export]
...
pub(crate) fn try_parse_dtype<S: AsRef<str>>(s: S) -> DaftResult<DataType> {
    let dialect = GenericDialect{};
    let tokens = Tokenizer::new(&dialect, s.as_ref())
        .tokenize()
        .map_err(from_tokenizer_error)?;
    let mut parser = Parser::new(&dialect)
        .with_options(ParserOptions { trailing_commas: true, ..Default::default()})
        .with_tokens(tokens);
    let dtype = parser
        .parse_data_type()
        .map_err(from_parser_error)?;
    sql_dtype_to_dtype(&dtype)
}

/// Parses a SQL map of name-type pairs into a new Schema
pub fn try_parse_schema(schema: HashMap<String, String>) -> DaftResult<Schema> {
    let mut fields = vec![];
...
fn sql_dtype_to_dtype(dtype: &sqlparser::ast::DataType) -> DaftResult<DataType> {
            ...
            "tensor" => match properties.as_slice() {
                ...
                [inner_dtype, rest @ ..] => {
                    ...
                    let rest = rest
                        ...
                        .map(|p| {
                            p.parse().map_err(|_| {
                                DaftError::ValueError("invalid tensor shape".to_string())
                            })
                        })
                        .collect::<DaftResult<Vec<_>>>()?;
            ...
            "image" => match properties.as_slice() {
                ...
                [mode] => {
                    let mode = mode.parse().map_err(|_| {
                        DaftError::ValueError("invalid image mode".to_string())
                    })?;
                ...
                [mode, height, width] => {
                    ...
                    let height = height.parse().map_err(|_| {
                        DaftError::ValueError("invalid image height".to_string())
                    })?;
                    let width = width.parse().map_err(|_| {
                        DaftError::ValueError("invalid image width".to_string())
                    })?;
...
/// Converts a time precision to a TimeUnit e.g 3 -> milli, 6 -> micro, 9 -> nano.
fn timeunit_from_precision(prec: Option<u64>) -> DaftResult<TimeUnit> {
    Ok(match prec {
...
/// Any input error becomes a Daft ValueError and don't distinguish between 'lexer' vs 'parser' errors.
fn from_tokenizer_error(err: TokenizerError) -> DaftError {
    DaftError::ValueError(format!("Failed to parse the DataType, {}", err))
}

/// Any input error becomes a Daft ValueError.
fn from_parser_error(err: ParserError) -> DaftError {
    DaftError::ValueError(format!("Failed to parse the DataType, {}", err))
}
...
mod test {
    ...
    use crate::{dtype::DataType, prelude::ImageMode};
