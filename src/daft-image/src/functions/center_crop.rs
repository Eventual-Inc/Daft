...
#[typetag::serde]
impl ScalarUDF for ImageCenterCrop {
    fn call(&self, inputs: daft_dsl::functions::FunctionArgs<Series>) -> DaftResult<Series> {
        ...
        crate::series::center_crop(input, width, height)
    }
    ...
    fn get_return_field(
        ...
        let height = inputs.required(("h", "height")).and_then(|e| {
            ...
            e.as_literal()
                ...
                .ok_or_else(|| {
                    DaftError::ValueError("height must be a positive integer".to_string())
                })
        })?;

        match field.dtype {
            DataType::Image(_) => Ok(field),
            DataType::FixedShapeImage(mode, h0, w0) => {
                let out_height = height.min(h0);
                let out_width = width.min(w0);
                Ok(Field::new(
                    field.name,
                    DataType::FixedShapeImage(mode, out_height, out_width),
                ))
            }
            _ => Err(DaftError::TypeError(format!(
                "ImageCenterCrop can only center crop ImageArrays and FixedShapeImageArrays, got {field}",
            ))),
        }
    }

    fn docstring(&self) -> &'static str {
        "Center crops an image around its center to the specified width and height."
    }
}
