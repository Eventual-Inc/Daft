    ...
    pub async fn create_writer(file_ref: FileReference) -> DaftResult<Self> {
            ...
            .map_err(DaftError::from)?
        {
            Some(mut multipart_writer) => {
                ...
                // Only complete (commit) the upload if the writer was closed cleanly; any
                // other outcome aborts the upload so the store does not retain (and bill
                // for) parts of an incomplete multipart upload.
                let upload_task = common_runtime::get_io_runtime(true).spawn(async move {
                    ...
                    .await;
                    if (result.is_err() || !should_commit.load(Ordering::SeqCst))
                        && let Err(abort_err) = multipart_writer.abort().await
                    {
                        log::warn!("Failed to abort incomplete multipart upload: {abort_err}");
                    }
                    result
                });
        ...
        match writer {
            FileWriter::Multipart {
                ...
                while part_buffer.len() >= *part_size {
                    ...
                    let rt = common_runtime::get_io_runtime(true);
                    let sent =
                        rt.block_within_async_context(async move { sender.send(part).await })?;
                    if sent.is_err() {
                        upload_died = true;
                        break;
                    }
...
pub(crate) enum FileWriter {
    ...
    /// drains parts in the background, completes the multipart upload when the commit
    /// flag is set (i.e. the writer was closed cleanly), and aborts it otherwise. All
    /// sends go through `block_within_async_context` since writes may be issued from
    /// executor threads.
    Multipart {
        part_buffer: Vec<u8>,
        part_size: usize,
