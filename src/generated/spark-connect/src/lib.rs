...
/// client to the server. A [Plan](Plan) can either be of the type [Relation](Relation) which is a reference
/// to the underlying logical plan or it can be of the [Command](Command) type that is used to execute
