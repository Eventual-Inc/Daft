...
impl Offset {
    ...
    pub(crate) fn with_materialized_stats(mut self) -> Self {
        ...
        let slice_selectivity = if approx_num_rows > 0 {
        ...
        let approx_stats = ApproxStats {
            num_rows: offset_num_rows.min(approx_num_rows),
            size_bytes: if approx_num_rows > offset_num_rows {
                let est_bytes_per_row =
                    input_stats.approx_stats.size_bytes / approx_num_rows.max(1);
                offset_num_rows * est_bytes_per_row
            } else {
