...
impl OptimizerRule for SplitExplodeFromProject {
    fn try_optimize(&self, plan: LogicalPlanRef) -> DaftResult<Transformed<LogicalPlanRef>> {
        plan.transform_down(|node| {
            ...
            let mut ignore_empty: Option<bool> = None;
            ...
            let new_projection = projection
                ...
                .map(|expr| {
                    ...
                    {
                        let current_ignore_empty = if sf.inputs.len() == 2 {
                            match sf
                                .inputs
                                .iter()
                                .nth(1)
                            ...
                            {
                                Some(b) => b,
                                None => return Ok(expr.clone()), // Cannot optimize non-literal ignore_empty
                            }
                        } else {
                            false
                        };

                        if let Some(d) = ignore_empty {
                            if d != current_ignore_empty {
                                // Conflicting ignore_empty flags, cannot optimize into single Explode
                                return Ok(expr.clone());
                            }
                        } else {
                            ignore_empty = Some(current_ignore_empty);
                        }
            ...
            if to_explode.is_empty() {
                ...
                // Explode columns with inconsistent `ignore_empty` flags are skipped (kept as scalar
                // functions in the projection). Only columns with consistent `ignore_empty` are
