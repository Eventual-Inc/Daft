    ...
    fn try_optimize_project(
        ...
        match upstream_plan.as_ref() {
            ...
            LogicalPlan::AsofJoin(asof_join) => {
                ...
                let maybe_project = |side: &LogicalPlanRef,
                                     side_dependencies: &IndexSet<String>|
                 -> DaftResult<Transformed<LogicalPlanRef>> {
                    let schema = side.schema();
                    let upstream_names: IndexSet<String> =
                ...
                let new_left_upstream = maybe_project(&asof_join.left, &left_dependencies)?;
                let new_right_upstream = maybe_project(&asof_join.right, &right_dependencies)?;

                if !new_left_upstream.transformed && !new_right_upstream.transformed {
                    Ok(Transformed::no(plan))
                } else {
                    let new_asof_join = upstream_plan
                        .with_new_children(&[new_left_upstream.data, new_right_upstream.data]);

                    let new_plan = Arc::new(plan.with_new_children(&[new_asof_join.into()]));
