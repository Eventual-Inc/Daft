...
mod tests {
    ...
    fn test_offset_greater_than_limit() -> DaftResult<()> {
        ...
        let expected = dummy_scan_node(scan_op)
            .limit_with_offset(0, None, false)?
            .build();
    ...
    fn test_merge_limit_offset(#[values(false, true)] none_offset: bool) -> DaftResult<()> {
        ...
        let plan = dummy_scan_node(scan_op.clone())
            .limit_with_offset(limit, if none_offset { None } else { Some(1) }, false)?
            .offset(offset)?
            .build();
        let expected = dummy_scan_node(scan_op)
            .limit_with_offset(10, if none_offset { Some(7) } else { Some(8) }, false)?
            .build();
        assert_optimized_plan_eq(plan, expected)
    }
    ...
    fn test_merge_limit_with_greater_offset() -> DaftResult<()> {
        ...
        let plan = dummy_scan_node(scan_op.clone())
            .limit_with_offset(limit, Some(5), false)?
            .offset(offset)?
            .build();
        let expected = dummy_scan_node(scan_op).limit(0, false)?.build();
        assert_optimized_plan_eq(plan, expected)
    }
