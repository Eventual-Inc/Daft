    ...
    pub fn with_new_children(&self, children: &[Arc<Self>]) -> Self {
        match children {
            [input] => match self {
                ...
                Self::Limit(Limit { offset, limit, eager, .. }) => Self::Limit(Limit::try_new(input.clone(), *offset, *limit, *eager).unwrap()),
                ...
                Self::TopN(TopN { sort_by, descending, nulls_first, offset, limit, .. }) => Self::TopN(TopN::try_new(
                    input.clone(), sort_by.clone(), descending.clone(), nulls_first.clone(), *offset, *limit
                ).unwrap()),
                Self::Concat(_)
                | Self::Intersect(_)
                | Self::Union(_)
