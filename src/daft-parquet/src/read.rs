...
impl std::str::FromStr for StringEncoding {
    type Err = daft_common_error::DaftError;

    fn from_str(value: &str) -> Result<Self, Self::Err> {
        match value {
            "utf-8" => Ok(Self::Utf8),
            "raw" => Ok(Self::Raw),
            other => Err(daft_common_error::DaftError::ValueError(format!(
    ...
    fn try_from(value: ParquetSchemaInferenceOptionsBuilder) -> crate::Result<Self> {
        let string_encoding: StringEncoding =
            value
                .string_encoding
                .parse()
                .map_err(|e: daft_common_error::DaftError| crate::Error::ArrowError {
