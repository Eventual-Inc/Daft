...
fn make_decimal_column_range_statistics(
    ...
    let lower =
        Decimal128Array::from_iter_and_fld(lower_field, std::iter::once(Some(l))).into_series();
    let upper =
        Decimal128Array::from_iter_and_fld(upper_field, std::iter::once(Some(u))).into_series();
