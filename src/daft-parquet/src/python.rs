...
pub fn read_parquet_statistics(
    ...
    py.allow_threads(|| {
        ...
        Ok(
            crate::read::read_parquet_statistics(&uris.series, io_client, Some(io_stats), None)?
                .into(),
        )
    })
}
...
pub fn register_modules(parent: &Bound<PyModule>) -> PyResult<()> {
    parent.add_function(wrap_pyfunction_bound!(read_parquet, parent)?)?;
    parent.add_function(wrap_pyfunction_bound!(read_parquet_into_pyarrow, parent)?)?;
    parent.add_function(wrap_pyfunction_bound!(
        read_parquet_into_pyarrow_bulk,
        parent
    )?)?;
    parent.add_function(wrap_pyfunction_bound!(read_parquet_bulk, parent)?)?;
    parent.add_function(wrap_pyfunction_bound!(read_parquet_schema, parent)?)?;
    parent.add_function(wrap_pyfunction_bound!(read_parquet_statistics, parent)?)?;
    Ok(())
}
