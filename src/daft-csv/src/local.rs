    ...
    fn new(states: &'a [ChunkState]) -> Self {
        ...
        for state in states {
            let slice = match state {
                ChunkState::Start { slab, start } => {
                    &slab.buffer.as_ref().unwrap()[*start..slab.valid_bytes]
                }
                ChunkState::StartAndFinal { slab, start, end } => {
                    &slab.buffer.as_ref().unwrap()[*start..*end]
                }
                ChunkState::Continue { slab } => {
                    &slab.buffer.as_ref().unwrap().as_ref()[..slab.valid_bytes]
                }
                ChunkState::Final { slab, end } => &slab.buffer.as_ref().unwrap().as_ref()[..*end],
            };
            slices.push(slice);
        }
    ...
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        ...
        while self.curr_read_idx < self.slices.len() && position < buf_len {
            ...
            if self.curr_read_offset < slice.len() {
                let read_size = buf_len.min(slice.len() - self.curr_read_offset);
