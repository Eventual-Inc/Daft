...
struct SimpleMetricsService {
    operator_metrics: DashMap<u64, (String, DashMap<String, Value>)>,
}

impl SimpleMetricsService {
    pub fn new() -> Self {
        Self {
            operator_metrics: DashMap::new(),
        }
    }
        ...
        for resource_metric in req.resource_metrics {
            for scope_metrics in resource_metric.scope_metrics {
                for metric in scope_metrics.metrics {
                    ...
                    let operator_name = pieces[2];
                    let metric_name = pieces[3];
                    let metrics = self
                        .operator_metrics
                        .entry(operator_id)
                        .or_insert((operator_name.to_string(), DashMap::new()));

                    let value = if let Some(data) = metric.data {
                        use opentelemetry_proto::tonic::metrics::v1::metric::Data;
                        match data {
                            Data::Gauge(g) => g.data_points.last().and_then(|p| p.value),
                            Data::Sum(s) => s.data_points.last().and_then(|p| p.value),
                    ...
                    if let Some(value) = value {
                        metrics.1.insert(metric_name.to_string(), value);
                    }
                }
            }

            for op_id in 1..=6 {
                let op_entry = self.operator_metrics.get(&op_id).unwrap();
                let (name, metrics) = op_entry.value();
                let metrics_str = metrics
                    ...
                    .collect::<Vec<String>>()
                    .join(", ");
                println!("[{}/6] {}: {}", op_id, name, metrics_str);
            }
        }
...
pub async fn serve_otlp_metrics(
    addr: SocketAddr,
