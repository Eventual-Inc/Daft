    ...
    match logical_plan {
        LogicalPlan::Join(LogicalJoin {
            ...
            match join_strategy {
                ...
                // we decided not to do broadcast join, but our children have not been materialized, so we need to materialize them to get stats
                JoinStrategy::SortMerge | JoinStrategy::Hash
                    if !left_is_in_memory || !right_is_in_memory =>
                {
                    if left_is_in_memory {
                        // if left is in memory it means the right side is not in memory, so we should emit the right side first
                        let ph = PlaceHolderInfo::new(right.schema());
                        let new_scan = LogicalPlan::Source(Source::new(
                            right.schema(),
                            SourceInfo::PlaceHolder(ph).into(),
                        ));
                        let new_join =
                            logical_plan.with_new_children(&[left.clone(), new_scan.into()]);
                        ...
                        let ph = PlaceHolderInfo::new(left.schema());
                        let new_scan = LogicalPlan::Source(Source::new(
                            left.schema(),
                            SourceInfo::PlaceHolder(ph).into(),
                        ));
                        let new_join =
                            logical_plan.with_new_children(&[new_scan.into(), right.clone()]);
