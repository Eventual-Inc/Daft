            ...
            .map(move |t| -> DaftResult<BoxScanTaskIter<'a>> {
                ...
                if let (FileFormatConfig::Json(JsonSourceConfig { .. }), [source], Some(None)) = (
                    ...
                    if size_bytes <= cfg.scan_tasks_max_size_bytes
                        || size_bytes < cfg.scan_tasks_min_size_bytes
                    {
                        return Ok(Box::new(std::iter::once(Ok(t))));
                    }
                    ...
                    while pos < size_bytes {
                        let target = pos + cfg.scan_tasks_max_size_bytes;
                        let mut end = align_right(target.min(size_bytes))?;
                        if end <= pos {
                            // Fallback: advance by finding the next newline from pos + 1
                            end = align_right((pos + 1).min(size_bytes))?;
                            if end <= pos {
                                break;
                            }
                        }
...
fn resolve_source_size(
    ...
    if let Some(size) = size_hint {
        return usize::try_from(size).map_err(|_| {
            DaftError::ValueError(format!("ScanTask size {size} exceeds usize::MAX"))
        });
    }
    ...
    if let Some(local_path) = local_path_from_uri(path) {
        ...
        return usize::try_from(file_size).map_err(|_| {
            DaftError::ValueError(format!(
                "File {} size {file_size} exceeds usize::MAX",
                local_path.display()
            ))
        });
    }
