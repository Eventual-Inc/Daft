                            ...
                            while let Some(file_metadata) = all_paths.next().await {
                                ...
                                .await
                                {
                                    Ok((schema, read_stats)) => {
                                        // Treat files with zero data rows as corrupt when ignore_corrupt_files is set.
                                        if read_stats.total_records_read > 0 {
                                            schema_opt = Some(schema);
                                            break;
                                        } else {
                                            log::warn!(
                                                "Skipping unreadable/corrupt csv file {}: no data rows detected",
                                                fp,
                                            );
                                        }
                                    }
