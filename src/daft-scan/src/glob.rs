            ...
            true => {
                let (inferred_schema, first_metadata) = match file_format_config.as_ref() {
                    &FileFormatConfig::Parquet(ParquetSourceConfig {
                        ...
                        let ignore_flag = matches!(file_format_config.as_ref(), FileFormatConfig::Parquet(ParquetSourceConfig { ignore_error: true, .. }));
                        if ignore_flag {
                            ...
                            while let Some(file_metadata) = all_paths.next().await {
                                ...
                                .await
                                {
                                    Ok((schema, metadata)) => {
                                        ...
                                        first_md_opt = Some((
                                            fp,
                                            TableMetadata { length: metadata.num_rows },
                                        ));
                                        break;
                                    }
                                    Err(e) => {
                                        log::warn!("Skipping unreadable/corrupt parquet file {}: {}", fp, e);
                                        continue;
                                    }
                                }
                            }
                    ...
                    FileFormatConfig::Csv(CsvSourceConfig {
                        ...
                        let ignore_flag = matches!(file_format_config.as_ref(), FileFormatConfig::Csv(CsvSourceConfig { ignore_error: true, .. }));
                        if ignore_flag {
                            let mut all_paths = run_glob(
                                first_glob_path.clone(),
                                None,
                            ...
                            while let Some(file_metadata) = all_paths.next().await {
                                ...
                                .await
                                {
                                    ...
                                    Err(e) => {
                                        log::warn!("Skipping unreadable/corrupt csv file {}: {}", fp, e);
                                        continue;
                                    }
                                }
                            }
