...
fn parse_filter(filter: Filter) -> Result<LogicalPlanBuilder> {
    ...
    let condition = expression::convert_expression(condition).wrap_err("converting to daft expression")?;
...
fn parse_with_columns(with_columns: WithColumns) -> Result<LogicalPlanBuilder> {
    ...
    for alias in aliases {
        ...
        let expr = expression::convert_expression(*expr).wrap_err("converting to daft expression")?;
