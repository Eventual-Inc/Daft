...
pub fn to_logical_plan(plan: Relation) -> Result<LogicalPlanBuilder> {
    ...
    match rel_type {
        RelType::ShowString(..) => bail!("ShowString is only supported as a top-level relation"),
...
fn parse_filter(filter: Filter) -> Result<LogicalPlanBuilder> {
    ...
    let condition = expr::to_daft_expr(condition).wrap_err("converting to daft expression")?;
...
fn parse_with_columns(with_columns: WithColumns) -> Result<LogicalPlanBuilder> {
    ...
    for alias in aliases {
        ...
        let expr = expr::to_daft_expr(*expr).wrap_err("converting to daft expression")?;
...
fn parse_read(read: Read) -> Result<LogicalPlanBuilder> {
    ...
    if is_streaming {
        bail!("Streaming reads are not yet supported");
    }
