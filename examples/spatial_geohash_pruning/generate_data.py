...
def geohash_batch(lons: np.ndarray, lats: np.ndarray, precision: int = 5) -> list[str]:
    ...
    min_lat = np.full(n, -90.0)
    max_lat = np.full(n, 90.0)
    min_lon = np.full(n, -180.0)
    max_lon = np.full(n, 180.0)
...
def _build_all_indexes(root: str, geom_col: str) -> None:
    ...
    result = daft.sql(f"BUILD SPATIAL INDEX ON '{root}' USING '{geom_col}' RESOLUTION 7 GLOB '*.parquet'")
    n_dirs = result.select("directory").distinct().count_rows()
    print(f"\n  Built spatial indexes in {n_dirs} partition dir(s) under '{root}'")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output", default=None, help="Output directory (default: same directory as this script)")
    p.add_argument(
        "--no-single-file",
        action="store_true",
        help="Skip writing backward-compat single-file points.parquet / polygons.parquet",
    )
...
def main() -> None:
    ...
    while written_pts < N_PTS:
        ...
        print(
            f"  batch {batch_num:3d}: {written_pts:>12,} / {N_PTS:,} pts "
            f"({100 * written_pts / N_PTS:.1f}%)  {rate / 1e6:.2f} M rows/s",
            end="\r",
        )
    ...
    while written_poly < N_POLY:
        ...
        print(
            f"  batch {batch_num:3d}: {written_poly:>12,} / {N_POLY:,} polys "
            f"({100 * written_poly / N_POLY:.1f}%)  {rate / 1e6:.2f} M rows/s",
            end="\r",
        )
    ...
    pts_files = sum(
        ...
        for d in os.listdir(pts_root)
        if os.path.isdir(os.path.join(pts_root, d))
    )
    poly_files = sum(
        len([f for f in os.listdir(os.path.join(poly_root, d)) if f.endswith(".parquet")])
        for d in os.listdir(poly_root)
        if os.path.isdir(os.path.join(poly_root, d))
    )
    ...
    print("  Spatial indexes built for all partition directories.")
    print("\nReady!  Run the examples:")
    print("  DAFT_RUNNER=native python spatial_pip_geohash_pruning.py")
    print("  DAFT_RUNNER=native python spatial_partition_index.py")
