...
async def launch_notebook_server(item: LaunchNotebookRequest, response: Response, token: str = Depends(token_auth_scheme)):
    ...
    create_user = requests.post(f"{JUPYTERHUB_SERVICE_ADDR}/users/{email}", headers={"Authorization": f"Bearer {JUPYTERHUB_TOKEN}"})

    create_server_response = requests.post(f"{JUPYTERHUB_SERVICE_ADDR}/users/{email}/server", headers={"Authorization": f"Bearer {JUPYTERHUB_TOKEN}"})
    if create_server_response.status_code // 100 != 2:
        response.status_code = status.HTTP_500_INTERNAL_ERROR
        return create_server_response.json()

    user_response = requests.get(f"{JUPYTERHUB_SERVICE_ADDR}/users/{email}", headers={"Authorization": f"Bearer {JUPYTERHUB_TOKEN}"})
