...
class SliceOffsetsFixes:
    ...
    @staticmethod
    def ensure_table_slice_offsets_are_propagated(arrow_table: pa.Table) -> pa.Table:
        """
        ...
        for idx, name in enumerate(arrow_schema.names):
            ...
            if SliceOffsetsFixes._chunked_array_needs_slice_offset_propagation(column):
                new_column = SliceOffsetsFixes._propagate_chunked_array_slice_offsets(column)
                arrow_table = arrow_table.set_column(idx, field, new_column)
        return arrow_table

    @staticmethod
    def ensure_chunked_array_slice_offsets_are_propagated(chunked_array: pa.ChunkedArray) -> pa.ChunkedArray:
        """
        Ensures that chunked-array-level slice offsets are properly propagated to child arrays
        to prevent them from being dropped upon record batch conversion and FFI transfer.
        """
        if SliceOffsetsFixes._chunked_array_needs_slice_offset_propagation(chunked_array):
            return SliceOffsetsFixes._propagate_chunked_array_slice_offsets(chunked_array)
        else:
            return chunked_array

    @staticmethod
    def ensure_array_slice_offsets_are_propagated(array: pa.Array) -> pa.Array:
        """
        Ensures that array-level slice offsets are properly propagated to child arrays
        to prevent them from being dropped upon record batch conversion and FFI transfer.
        """
        if SliceOffsetsFixes._array_needs_slice_offset_propagation(array):
            return SliceOffsetsFixes._propagate_array_slice_offsets(array)
        else:
            return array
    ...
    def _chunked_array_needs_slice_offset_propagation(chunked_array: pa.ChunkedArray) -> bool:
        ...
        if not pa.types.is_struct(chunked_array.type) and not pa.types.is_fixed_size_list(chunked_array.type):
            return False
        return any(SliceOffsetsFixes._array_needs_slice_offset_propagation(chunk) for chunk in chunked_array.chunks)
    ...
    def _array_needs_slice_offset_propagation(array: pa.Array) -> bool:
        ...
        if pa.types.is_struct(array.type):
            return SliceOffsetsFixes._struct_array_needs_slice_offset_propagation(array)
        elif pa.types.is_fixed_size_list(array.type):
            return SliceOffsetsFixes._fixed_size_list_array_needs_slice_offset_propagation(array)
        else:
            return False
    ...
    def _propagate_chunked_array_slice_offsets(chunked_array: pa.ChunkedArray) -> pa.ChunkedArray:
        ...
        for chunk in chunked_array.chunks:
            new_chunk = SliceOffsetsFixes._propagate_array_slice_offsets(chunk)
    ...
    def _propagate_array_slice_offsets(array: pa.Array) -> pa.Array:
        ...
        """
        assert SliceOffsetsFixes._array_needs_slice_offset_propagation(array)
        dtype = array.type
        if pa.types.is_struct(dtype):
            ...
            if bitmap_buf is not None:
                bitmap_buf = SliceOffsetsFixes._slice_bitmap_buffer(bitmap_buf, array.offset, len(array))
