...
def hash_chunked_array(arr, seed=None):
    ...
    if seed is not None:
        ...
        for chunk in arr.chunks:
            result_chunks.append(hash_pyarrow_array(chunk, pa, seed=seed.slice(offset, len(chunk)).combine_chunks()))
            offset += len(chunk)
    else:
        for chunk in arr.chunks:
            result_chunks.append(hash_pyarrow_array(chunk, pa, seed=None))
