...
TODO: Remove doctest +SKIP directives when KV backend implementations are complete.
The skip directives were added to allow the interface framework to pass precommit checks
while backend implementations (Lance, LMDB, etc.) are still in development.
"""
...
def kv_get(
    ...
    Examples:
        ...
        >>> from daft.kv import load_kv  # doctest: +SKIP
        >>> from daft.functions.kv import kv_get  # doctest: +SKIP
        >>> # Create KV stores with unique names for this doctest
        >>> import time  # doctest: +SKIP
        >>> timestamp = str(int(time.time() * 1000000))  # Microsecond timestamp for uniqueness  # doctest: +SKIP
        >>> embeddings_kv = load_kv("memory", name=f"embeddings_{timestamp}")  # doctest: +SKIP
        >>> metadata_kv = load_kv("memory", name=f"metadata_{timestamp}")  # doctest: +SKIP
        >>> daft.attach(embeddings_kv, alias=f"lance_embeddings_{timestamp}")  # doctest: +SKIP
        >>> daft.attach(metadata_kv, alias=f"lance_metadata_{timestamp}")  # doctest: +SKIP
        >>> daft.set_kv(f"lance_embeddings_{timestamp}")  # Set default  # doctest: +SKIP
        >>>
        >>> df = daft.from_pydict({"item_id": [0, 1, 2]})  # doctest: +SKIP
        >>> # All operations now return Struct type
        >>> df_struct_all = df.with_column("data", kv_get("item_id"))  # Returns struct with all fields  # doctest: +SKIP
        >>> # Get specific columns as struct
        >>> df_struct = df.with_column(  # doctest: +SKIP
        ...     "embedding_data",
        ...     kv_get("item_id", ["embedding"], source=f"lance_embeddings_{timestamp}"),  # doctest: +SKIP
        ... )  # doctest: +SKIP
...
def kv_batch_get(
    ...
    Examples:
        >>> import daft  # doctest: +SKIP
        >>> from daft.kv import load_kv  # doctest: +SKIP
        >>> from daft.functions.kv import kv_batch_get  # doctest: +SKIP
        >>> # Create KV stores with unique names for this doctest
        >>> import time  # doctest: +SKIP
        >>> timestamp = str(int(time.time() * 1000000))  # Microsecond timestamp for uniqueness  # doctest: +SKIP
        >>> embeddings_kv = load_kv("memory", name=f"embeddings_{timestamp}")  # doctest: +SKIP
        ...
        >>> daft.set_kv(f"lance_embeddings_{timestamp}")  # Set default  # doctest: +SKIP
        >>>
        >>> df = daft.from_pydict({"row_ids": [[0, 1], [2, 3]]})  # doctest: +SKIP
        >>> # Use default KV store
        >>> df = df.with_column("embeddings", kv_batch_get("row_ids"))  # doctest: +SKIP
        >>> # Use specific KV store via source parameter
        >>> df = df.with_column("metadata", kv_batch_get("row_ids", source=f"lance_metadata_{timestamp}"))  # doctest: +SKIP
    """
    from daft.expressions import lit
    from daft.session import current_kv, get_kv
...
def kv_exists(
    ...
    Examples:
        >>> import daft  # doctest: +SKIP
        >>> from daft.kv import load_kv  # doctest: +SKIP
        >>> from daft.functions.kv import kv_exists  # doctest: +SKIP
        >>> # Create KV stores with unique names for this doctest
        >>> import time  # doctest: +SKIP
        >>> timestamp = str(int(time.time() * 1000000))  # Microsecond timestamp for uniqueness  # doctest: +SKIP
        >>> embeddings_kv = load_kv("memory", name=f"embeddings_{timestamp}")  # doctest: +SKIP
        ...
        >>> daft.set_kv(f"lance_embeddings_{timestamp}")  # Set default  # doctest: +SKIP
        >>>
        >>> df = daft.from_pydict({"item_id": [0, 1, 999]})  # doctest: +SKIP
        >>> # Check existence in default KV store
        >>> df = df.with_column("exists_embeddings", kv_exists("item_id"))  # doctest: +SKIP
        >>> # Check existence in specific KV store via source parameter
        >>> df = df.with_column(
        ...     "exists_metadata", kv_exists("item_id", source=f"lance_metadata_{timestamp}")
        ... )  # doctest: +SKIP
    """
    from daft.expressions import lit
    from daft.session import current_kv, get_kv
