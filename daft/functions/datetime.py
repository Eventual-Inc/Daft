...
def convert_time_zone(expr: Expression, to_timezone: str, from_timezone: str | None = None) -> Expression:
    ...
    If the timestamp has no timezone, ``from_timezone`` must be provided to interpret
    the local time before converting to ``to_timezone``.
...
def replace_time_zone(expr: Expression, timezone: str | None) -> Expression:
    """Replaces the timezone of a timestamp while preserving the local time.

    Args:
        expr: Timestamp expression to update.
        timezone: New timezone (e.g. "UTC", "+02:00", "America/New_York"). Use None to remove timezone.
