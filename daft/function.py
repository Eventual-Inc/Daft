...
class QueryFunction(Protocol[ReturnType]):
    """A function that can be called on ColumnArgTypes, returning a QueryExpression"""

    def __call__(self, *args: ColumnArgType, **kwargs: ColumnArgType) -> UDFContext[ReturnType]:
        ...


@dataclasses.dataclass(frozen=True)
class UDFContext(Generic[ReturnType]):
    """A declaration of a function and the columns to use as input to the function"""

    func: UserFunction
    return_type: Type[ReturnType]
    args: Tuple[ColumnArgType, ...]
    kwargs: Dict[str, ColumnArgType]
...
def func(
    ...
    can be called on ColumnArgTypes to produce QueryExpressions, which can then be used in a Datarepo
