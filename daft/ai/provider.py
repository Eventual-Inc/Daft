...
def load_vllm_prefix_cached(name: str | None = None, **options: Any) -> Provider:
    try:
        ...
        return VLLMPrefixCachedProvider(name, **options)
    except ImportError as e:
        raise ProviderImportError(["vllm"]) from e


ProviderType = Literal["lm_studio", "openai", "transformers", "vllm-prefix-cached"]
PROVIDERS: dict[ProviderType, Callable[..., Provider]] = {
    "lm_studio": load_lm_studio,
    "openai": load_openai,
    "transformers": load_transformers,
    "vllm-prefix-cached": load_vllm_prefix_cached,
}
