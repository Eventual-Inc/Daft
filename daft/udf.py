...
class UDF:
    ...
    Calling this class produces a [daft.Expression][daft.expressions.Expression] that can be used in a DataFrame function.
...
def udf(
    *,
    ...
    !!! note "In most cases, UDFs will be slower than a native kernel/expression because of the required Rust and Python overheads."
        If your computation can be expressed using Daft expressions, you should do so instead of writing a UDF. If your UDF expresses a
    ...
    Example:
        ...
        5. We can call our UDF on a dataframe using any of the dataframe projection operations ([df.with_column()][daft.DataFrame.with_column],
        [df.select()][daft.DataFrame.select], etc.)
