...
class PyConverter:
    ...
    def _daft_type(self, field) -> DaftType | None:
...
class DaftSchema(Generic[_T]):
    ...
    def parse_dataclass(cls, t: type):
        ...
        for field in pydataclasses.fields(t):
            name = field.name
            pytype = field.type
            daft_field_metadata: DaftFieldMetadata | None = field.metadata.get(DaftFieldMetadata.__name__, None)
    ...
    def type_to_metadata(cls, t: type, conversion: bool) -> dict[str, bytes]:
        ...
        if hasattr(t, "__qualname__"):
            ...
            metadata["source_type"] = t._name.encode()  # type: ignore
        else:
