...
class UDF:
    ...
    def _validate_runtime_env(self) -> None:
        ...
        if get_context().get_or_infer_runner_type() == "native":
            raise ValueError("`@udf(runtime_env=...)` isn't currently supported in Native Runner.")

        if get_context().daft_execution_config.use_legacy_ray_runner:
            raise ValueError("`@udf(runtime_env=...)` isn't currently supported in Legacy Ray Runner.")

        # TODO(zhenchao) Support setting @udf(runtime_env=...) without concurrency parameter
        if self.concurrency is None:
            raise ValueError("The `concurrency` must also be set when setting the `runtime_env` option.")

        # TODO(zhenchao) Add support for more runtime_env options
        unsupported_keys = [key for key in self.runtime_env.keys() if key != "conda"]
        if unsupported_keys:
            unsupported_keys_str = ", ".join(unsupported_keys)
            raise ValueError(
                f"The `runtime_env` of UDF currently only supports configuring conda env, "
                f"but got '{unsupported_keys_str}'"
            )
