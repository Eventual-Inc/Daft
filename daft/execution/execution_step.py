...
@dataclass(frozen=True)
class PreShuffleMerge(FanoutInstruction):
    """
    Handles the merging of partitioned data from multiple map jobs before the shuffle phase.
    This class combines corresponding partitions across different map jobs into single partitions.

    For example, if you have 3 map jobs that each produced 2 partitions:
    Map1: [P0, P1]
    Map2: [P0, P1]
    Map3: [P0, P1]

    This will merge them into:
    [Merged_P0 (from Map1_P0 + Map2_P0 + Map3_P0),
     Merged_P1 (from Map1_P1 + Map2_P1 + Map3_P1)]
    """

    num_map_jobs: int  # Number of map jobs whose outputs need to be merged

    def run(self, inputs: list[MicroPartition]) -> list[MicroPartition]:
        """
        Entry point for the pre-shuffle merge operation.

        Args:
            inputs: List of all partitions from all map jobs

        Returns:
            List of merged partitions
        """
        return self._pre_shuffle_merge(inputs)

    def _pre_shuffle_merge(self, inputs: list[MicroPartition]) -> list[MicroPartition]:
        """
        Performs the actual merging of partitions from different map jobs.

        Args:
            inputs: List of all partitions from all map jobs. The list is ordered such that
                   all partitions from a single map job are consecutive, e.g.:
                   [map1_p0, map1_p1, map2_p0, map2_p1, map3_p0, map3_p1]

        Returns:
            List of merged partitions where each partition contains the combined data
            from corresponding partitions across all map jobs.

        Example:
            With 2 map jobs and 3 partitions each:
            inputs = [map1_p0, map1_p1, map1_p2, map2_p0, map2_p1, map2_p2]
            returns = [merged_p0, merged_p1, merged_p2]
            where merged_p0 = concat(map1_p0, map2_p0), etc.
        """
        # assert that it is evenly divisible
        assert len(inputs) % self.num_map_jobs == 0
        num_partitions_per_map = len(inputs) // self.num_map_jobs

        res = []
        # Process one partition number at a time
        for part in range(num_partitions_per_map):
            to_concat = []
            # For each partition number, gather the corresponding partition
            # from each map job's output
            for i in range(self.num_map_jobs):
                # Calculate index using: (map job index * partitions per job) + partition number
                partition_index = i * num_partitions_per_map + part
                to_concat.append(inputs[partition_index])

            # Merge all collected partitions with the same partition number
            concated = MicroPartition.concat(to_concat)
            res.append(concated)
    ...
    def run_partial_metadata(self, input_metadatas: list[PartialPartitionMetadata]) -> list[PartialPartitionMetadata]:
        """
        Computes the metadata for merged partitions by combining metadata from the input partitions.
        This function mirrors the merging logic of _pre_shuffle_merge but operates on metadata instead
        of actual data.

        Args:
            input_metadatas: List of metadata objects for all partitions from all map jobs,
                            ordered the same way as the actual partitions.

        Returns:
            List of metadata objects for the merged partitions, where each metadata object
            represents the combined stats of corresponding partitions across all map jobs.

        Example:
            With 2 map jobs, 2 partitions each, and input:
            [
                PartialPartitionMetadata(num_rows=100, size_bytes=1000),  # map1_p0
                PartialPartitionMetadata(num_rows=200, size_bytes=2000),  # map1_p1
                PartialPartitionMetadata(num_rows=150, size_bytes=1500),  # map2_p0
                PartialPartitionMetadata(num_rows=250, size_bytes=2500)   # map2_p1
            ]

            Returns:
            [
                PartialPartitionMetadata(num_rows=250, size_bytes=2500),  # merged_p0 (map1_p0 + map2_p0)
                PartialPartitionMetadata(num_rows=450, size_bytes=4500)   # merged_p1 (map1_p1 + map2_p1)
            ]
        """
        # Verify we have the expected number of input metadata objects
        assert len(input_metadatas) % self.num_map_jobs == 0
        num_partitions_per_map = len(input_metadatas) // self.num_map_jobs
        result = []
        # Process one partition number at a time
        for part in range(num_partitions_per_map):
            # Initialize accumulators for metadata
            total_rows = 0
            total_bytes = 0
            all_rows_known = True
            all_bytes_known = True

            # For each partition number, gather and sum metadata from corresponding
            # partitions across all map jobs
            for i in range(self.num_map_jobs):
                metadata = input_metadatas[i * num_partitions_per_map + part]
                ...
                if metadata.size_bytes is not None:
                    ...
                    all_bytes_known = False

            # Create merged metadata, using None if any input was None
            merged_metadata = PartialPartitionMetadata(
                num_rows=total_rows if all_rows_known else None, size_bytes=total_bytes if all_bytes_known else None
            )
            result.append(merged_metadata)
