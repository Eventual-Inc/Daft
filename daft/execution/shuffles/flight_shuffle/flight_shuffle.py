...
from daft.daft import (
    ...
    PyExpr,
    start_flight_server,
    reduce_partitions as py_reduce_partitions,
)
...
def get_shuffle_dir_path(
    ...
    return (
        f"{selected_dir}/daft_shuffle/node_{node_id}/shuffle_stage_{shuffle_stage_id}"
    )
...
class ShuffleActor:
    ...
    def __init__(
        ...
        self.shuffle_dir = get_shuffle_dir_path(
    ...
    def finish_push_partitions(self):
        ...
        self.server = start_flight_server(
            self.in_progress_shuffle_cache.close(), self.host
        )
    ...
    def shutdown(self):
        ...
        if self.server is not None:
            ...
            print(f"server shutdown time: {time.time() - start_time}")
        start_time = time.time()
        try:
        ...
        print(f"shuffle dir cleanup time: {time.time() - start_time}")
...
def reduce_partitions(
    ...
    try:
        ...
        partition_stream = py_reduce_partitions(shuffle_actor_addresses, partitions)
        next_partition_idx = 0
        while True:
            partition = loop.run_until_complete(partition_stream.next())
            if partition is None:
                break
            metadata = PartitionMetadata.from_table(partition)
            yield partition
            clear_partitions_futures.append(
                shuffle_actor_manager.clear_partition.remote(
                    partitions[next_partition_idx]
                )
            )
            yield metadata
        ...
        print("done clearing partitions")
...
def run_map_phase(
    ...
    while pending_map_tasks or in_flight_map_tasks:
        ...
        if len(done_ids) > 0:
            ...
            push_objects_futures.append(
                shuffle_actor_manager.push_objects.remote(done_partitions)
            )
...
def run_reduce_phase(
    ...
    reduce_task_to_partitions, partitions_to_reduce_task = (
        optimize_reduce_partition_assignment(actor_node_ids, num_output_partitions)
    )
