...
class DataFrame:
    ...
    def iter_rows(
        ...
        !!! tip "See also [df.iter_partitions][daft.DataFrame.iter_partitions] to iterator over entire partitions instead of single rows"
    ...
    def _from_arrow(cls, data: Union["pyarrow.Table", List["pyarrow.Table"], Iterable["pyarrow.Table"]]) -> "DataFrame":
        """Creates a DataFrame from a [pyarrow Table](https://arrow.apache.org/docs/python/generated/pyarrow.Table.html)."""
    ...
    def _from_pandas(cls, data: Union["pandas.DataFrame", List["pandas.DataFrame"]]) -> "DataFrame":
        """Creates a Daft DataFrame from a [pandas DataFrame](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.html)."""
        if not isinstance(data, list):
            data = [data]
    ...
    def unique(self) -> "DataFrame":
        ...
        !!! tip "See also [DataFrame.distinct][daft.DataFrame.distinct]"
        """
    ...
    def filter(self, predicate: Union[Expression, str]) -> "DataFrame":
        ...
        Alias for [DataFrame.where][daft.DataFrame.where].
        ...
        !!! tip "See also [DataFrame.where][daft.DataFrame.where]"
        """
    ...
    def join(
        ...
        !!! note "Although self joins are supported, we currently duplicate the logical plan for the right side and recompute the entire tree. Caching for this is on the roadmap."
    ...
    def explode(self, *columns: ColumnInputType) -> "DataFrame":
        ...
        If multiple columns are specified, each row must contain the same number of items in each specified column. Exploding Null values or empty lists will create a single Null entry (see example below).
    ...
    def unpivot(
        ...
        !!! tip "See also [melt][daft.DataFrame.melt]"
    ...
    def to_pandas(self, coerce_temporal_nanoseconds: bool = False) -> "pandas.DataFrame":
        ...
        Args:
            coerce_temporal_nanoseconds (bool): Whether to coerce temporal columns to nanoseconds. Only applicable to pandas version >= 2.0 and pyarrow version >= 13.0.0. Defaults to False. See [pyarrow.Table.to_pandas][https://arrow.apache.org/docs/python/generated/pyarrow.Table.html#pyarrow.Table.to_pandas] for more information.
    ...
    def to_pylist(self) -> List[Any]:
        ...
        !!! warning "This is a convenience method over [DataFrame.iter_rows()][daft.DataFrame.iter_rows]. Users should prefer using [.iter_rows()][daft.DataFrame.iter_rows] directly instead for lower memory utilization if they are streaming rows out of a DataFrame and don't require full materialization of the Python list.
        ...
        Example:
            ...
            ```
        !!! tip "See also [df.iter_rows()][daft.DataFrame.iter_rows] for streaming iterator over individual rows in a DataFrame
    ...
    def to_torch_iter_dataset(self) -> "torch.utils.data.IterableDataset":
        """Convert the current DataFrame into a [Torch IterableDataset](https://pytorch.org/docs/stable/data.html#torch.utils.data.IterableDataset) for use with PyTorch.
    ...
    def to_dask_dataframe(
        ...
        The returned Dask DataFrame will use [Dask-on-Ray](https://docs.ray.io/en/latest/ray-more-libs/dask-on-ray.html) to execute operations on a Ray cluster.
