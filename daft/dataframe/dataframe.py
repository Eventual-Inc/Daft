...
class DataFrame:
    ...
    def merge_columns(
        ...
        reader_schema: Optional["pyarrow.Schema"] = None,
        io_config: Optional[IOConfig] = None,
        ...
        right_on: str = "_rowaddr",
    ) -> None:
        """Directly merges new columns from the current DataFrame into the specified Lance dataset (without scanning the dataset).

        Requirements:
        - DataFrame must contain the 'fragment_id' fragment identifier column;
        - DataFrame must already contain the new columns to write back, along with the row address key for joining (defaults to '_rowaddr', which can be injected by the reader view);
        - The method groups by fragment, constructs a RecordBatchReader from the grouped data, and calls fragment.merge with row_addr as the join key to commit.

        Notes:
        - No transform parameter is needed; new columns should be generated through regular DataFrame computations before calling.
        - This implementation does not scan Lance, relying entirely on grouped data.
        """
        if "fragment_id" not in self.column_names:
            raise ValueError("DataFrame must contain 'fragment_id' column")
        ...
        merge_columns_from_df(
            ...
            batch_size=daft_remote_args.get("batch_size") if daft_remote_args else None,
            left_on=left_on,
            right_on=right_on,
        )
