...
def _convert_paimon_catalog_options_to_io_config(catalog_options: dict) -> IOConfig | None:
    ...
    io_config = IOConfig(
        s3=S3Config(
            endpoint_url=get("fs.s3.endpoint") or get("fs.oss.endpoint"),
            region_name=get("fs.s3.region") or get("fs.oss.region"),
            key_id=get("fs.s3.accessKeyId") or get("fs.oss.accessKeyId"),
            access_key=get("fs.s3.accessKeySecret") or get("fs.oss.accessKeySecret"),
            session_token=get("fs.s3.securityToken") or get("fs.oss.securityToken"),
        ),
    )
