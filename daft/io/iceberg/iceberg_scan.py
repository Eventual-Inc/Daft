...
class IcebergScanOperator(ScanOperator):
    def __init__(
        self, iceberg_table: Table, snapshot_id: int | None, storage_config: StorageConfig, ignore_corrupt_files: bool = False
    ) -> None:
        super().__init__()
        ...
        for task in iceberg_tasks:
            ...
            if file_format == "PARQUET":
                file_format_config = FileFormatConfig.from_parquet_config(
                    ParquetSourceConfig(field_id_mapping=self._field_id_mapping, ignore_corrupt_files=self._ignore_error)
                )
            else:
