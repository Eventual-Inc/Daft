...
def merge_columns_from_df(
    ...
    *,
    ...
    num_gpus = daft_remote_args.get("num_gpus", _UnsetMarker)
    memory_bytes = daft_remote_args.get("memory_bytes", _UnsetMarker)
    effective_batch_size = batch_size if batch_size is not None else daft_remote_args.get("batch_size", _UnsetMarker)
    ...
    # Build UDF expression and run grouped plan with per-fragment merge
    handler_udf = (
        GroupFragmentMergeUDF.with_init_args(lance_ds, left_on, right_on, read_columns, reader_schema, batch_size)  # type: ignore[attr-defined]
        .override_options(
            num_cpus=num_cpus, num_gpus=num_gpus, memory_bytes=memory_bytes, batch_size=effective_batch_size
        )
        .with_concurrency(concurrency)
    )
