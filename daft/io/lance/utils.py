...
def distribute_fragments_balanced(fragments: list[Any], fragment_group_size: int) -> list[dict[str, list[int]]]:
    ...
    group_num = max(1, len(fragments) // fragment_group_size)
    ...
    fragment_group_list: list[list[int]] = [[] for _ in range(group_num)]
    group_size_list = [0] * group_num
    ...
    for frag_info in fragment_info:
        ...
        min_workload_idx = min(range(group_num), key=lambda i: group_size_list[i])
